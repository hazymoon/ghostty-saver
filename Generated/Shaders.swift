...
// shaders-sha256: 508f87fcca7d0a710c2d2b8111ced05b1456cb7c4c84755bda610d97672fd73c
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _501[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
{
    return smoothstep(start + 0.300000011920928955078125, start + 5.0, u) - smoothstep(start + 5.5, (start + 10.0) - 0.20000000298023223876953125, u);
}
...
{
    float param = start + 0.300000011920928955078125;
    float param_1 = start + 5.0;
    float param_2 = u;
...
Pose cameraPose(thread const float& t)
{
    ...
    float param_19 = stepNo;
    float param_20 = stepNo + 1.0;
    float amp = mix(stepSize(param_19), stepSize(param_20), su) * gait;
    ...
    float param_21 = sinceStrike;
    float param_22 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + ((0.0040000001899898052215576171875 * gait) * ring(param_21, param_22));
    float param_23 = sinceStrike;
    float param_24 = 0.070000000298023223876953125;
    float bobRate = ((((0.02500000037252902984619140625 * amp) * arch) * stepRate) * (((6.283185482025146484375 * sin(6.283185482025146484375 * su)) + (2.51327419281005859375 * sin(12.56637096405029296875 * su))) + (0.94247782230377197265625 * sin(18.84955596923828125 * su)))) + (((0.0040000001899898052215576171875 * gait) * ringRate(param_23, param_24)) * moving);
    ...
    float param_25 = sinceStrike;
    float param_26 = 0.07999999821186065673828125;
    float kick = (0.001700000022538006305694580078125 * gait) * ring(param_25, param_26);
    float param_27 = sinceStrike;
    float param_28 = 0.07999999821186065673828125;
    float kickRate = ((0.001700000022538006305694580078125 * gait) * ringRate(param_27, param_28)) * moving;
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _910 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _910 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _922 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _922 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_910, _922);
    float3 _1008;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1008 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1008 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1008;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _813[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1055 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1055 = float2(0.0);
        }
        float2 owner = cell - _1055;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1226 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1226 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1226;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1250 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1250;
    ...
    if (rd.x == 0.0)
    {
        _1294 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1294 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1304 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1304 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1294, _1304);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1358 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1358;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1373 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1373;
...
{
    float2 param = cell;
    bool _1475 = !inBlackout(param);
    bool _1483;
    if (_1475)
    {
        float2 param_1 = cell;
        _1483 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1483 = _1475;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_1803 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2736)
{
    float2 res = float2(_2736.iResolution[0], _2736.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_2736.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _2736.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    ...
    int param_10;
    float3 param_11;
    float _3040 = trace(param_8, param_9, param_10, param_11);
    int id = param_10;
    float3 n = param_11;
    float dist = _3040;
    float3 p = ro + (rd * dist);
    ...
    float param_15 = tNow;
    float3 param_16;
    float3 _3060 = surface(param_12, param_13, param_14, param_15, param_16);
...
fragment main0_out main0(constant Globals& _2736 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _2736);
    out._fragColor = param;
    return out;
}
"""#
