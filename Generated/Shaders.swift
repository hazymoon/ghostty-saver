...
// shaders-sha256: dc8220cbc8d1fb1582fdaf38744e62add071f2715ef83028daf1abca2cf27dc8
// prefix-ref: v1.3.1
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2164 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3275)
{
    float2 res = float2(_3275.iResolution[0], _3275.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3275.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3275.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_14;
    float3 param_15;
    float _3589 = trace(param_12, param_13, param_14, param_15);
    int id = param_14;
    float3 n = param_15;
    float dist = _3589;
    float3 p = ro + (rd * dist);
    ...
    float3 param_21;
    float3 _3623 = surface(param_16, param_17, param_18, param_19, param_20, param_21);
    float3 emission = param_21;
    float3 albedo = _3623;
    float3 param_22 = p;
    float3 param_23 = n;
    float param_24 = tNow;
    float3 col = (albedo * (lighting(param_22, param_23, param_24) + float3(0.01200000010430812835693359375))) + emission;
...
fragment main0_out main0(constant Globals& _3275 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3275);
    out._fragColor = param;
    return out;
}
"""#
