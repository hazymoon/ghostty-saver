...
// shaders-sha256: cf3f6b2ab5bf740fc5a09eaaf7fdbf51874f07dad425c83d1221124cdfb9438a
// prefix-ref: v1.3.1
...
{
    float2 toDriven = _302[driven] - _302[driver];
    float alpha = precise::atan2(toDriven.y, toDriven.x);
    float fi = ((alpha - driverAngle) * _257[driver]) / 6.283185482025146484375;
    float fj = 0.5 - fi;
    return (alpha + 3.1415927410125732421875) - ((fj * 6.283185482025146484375) / _257[driven]);
}

static inline __attribute__((always_inline))
float pitchRadius(thread const int& i)
{
    return (0.010999999940395355224609375 * _257[i]) * 0.5;
}
...
static inline __attribute__((always_inline))
float hatchScale(constant Globals& _76)
{
    return fast::max(1.0, floor(_76.iResolution[1u] / 720.0));
}
...
static inline __attribute__((always_inline))
float hatch(thread const float& luminance, thread const float2& direction, thread const float2& fragCoord, constant Globals& _76)
{
    float2 p = fragCoord / float2(hatchScale(_76));
    float spacing = 7.0;
    float angle = precise::atan2(direction.y, direction.x);
    angle = (floor(((angle * 12.0) / 3.1415927410125732421875) + 0.5) * 3.1415927410125732421875) / 12.0;
    float2 d1 = float2(cos(angle), sin(angle));
...
static inline __attribute__((always_inline))
float wheel(thread const int& i, thread const float& angle, thread const float2& uv, thread const float2& fragCoord, thread const float& pixel, thread const float2& light, constant Globals& _76)
{
    int param = i;
    float r = pitchRadius(param);
    ...
    float theta = precise::atan2(p.y, p.x) - angle;
    float u = fract((theta * _257[i]) / 6.283185482025146484375);
    float root = r - 0.010999999940395355224609375;
    float param_1 = u;
    float radius = root + ((outer - root) * toothProfile(param_1));
    ...
    float strokeAngle = 0.60000002384185791015625 + (float(i) * 0.5);
    float2 stroke = float2(cos(strokeAngle), sin(strokeAngle));
    float param_2 = tone;
    float2 param_3 = stroke;
    float2 param_4 = fragCoord;
    float shade = ((hatch(param_2, param_3, param_4, _76) * 0.60000002384185791015625) * inside) * (1.0 - window);
    return fast::max(fast::max(outline, hubLine * inside), fast::max(windowLine * inside, shade));
}

static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _76)
{
    float2 uv = (fragCoord - (float2(_76.iResolution[0], _76.iResolution[1]) * 0.5)) / float2(_76.iResolution[1u]);
    uv.y = -uv.y;
    float pixel = 1.0 / _76.iResolution[1u];
    float land = fract(_76.iTime * 2.0);
    float settle = 1.0 - (exp((-land) * 16.0) * (1.0 - (0.3499999940395355224609375 * sin(land * 40.0))));
    float escape = ((floor(_76.iTime * 2.0) + settle) * 6.283185482025146484375) / 24.0;
    float angleA = _76.iTime * 0.3499999940395355224609375;
    int param = 1;
    int param_1 = 0;
    float param_2 = angleA;
    float angleB = phaseFor(param, param_1, param_2);
    int param_3 = 2;
    int param_4 = 1;
    float param_5 = angleB;
    float angleC = phaseFor(param_3, param_4, param_5);
    int param_6 = 3;
    int param_7 = 2;
    float param_8 = angleC;
    float angleD = phaseFor(param_6, param_7, param_8);
    int param_9 = 4;
    int param_10 = 3;
    float param_11 = angleD;
    float angleE = phaseFor(param_9, param_10, param_11);
    float angleF = escape;
    int param_12 = 6;
    int param_13 = 5;
    float param_14 = angleF;
    float angleG = phaseFor(param_12, param_13, param_14);
    float2 light = float2(-0.60000002384185791015625, 0.800000011920928955078125);
    float coverage = 0.0;
    int param_15 = 0;
    float param_16 = angleA;
    float2 param_17 = uv;
    float2 param_18 = fragCoord;
    float param_19 = pixel;
    float2 param_20 = light;
    coverage = fast::max(coverage, wheel(param_15, param_16, param_17, param_18, param_19, param_20, _76));
    int param_21 = 1;
    float param_22 = angleB;
    float2 param_23 = uv;
    float2 param_24 = fragCoord;
    float param_25 = pixel;
    float2 param_26 = light;
    coverage = fast::max(coverage, wheel(param_21, param_22, param_23, param_24, param_25, param_26, _76));
    int param_27 = 2;
    float param_28 = angleC;
    float2 param_29 = uv;
    float2 param_30 = fragCoord;
    float param_31 = pixel;
    float2 param_32 = light;
    coverage = fast::max(coverage, wheel(param_27, param_28, param_29, param_30, param_31, param_32, _76));
    int param_33 = 3;
    float param_34 = angleD;
    float2 param_35 = uv;
    float2 param_36 = fragCoord;
    float param_37 = pixel;
    float2 param_38 = light;
    coverage = fast::max(coverage, wheel(param_33, param_34, param_35, param_36, param_37, param_38, _76));
    int param_39 = 4;
    float param_40 = angleE;
    float2 param_41 = uv;
    float2 param_42 = fragCoord;
    float param_43 = pixel;
    float2 param_44 = light;
    coverage = fast::max(coverage, wheel(param_39, param_40, param_41, param_42, param_43, param_44, _76));
    int param_45 = 5;
    float param_46 = angleF;
    float2 param_47 = uv;
    float2 param_48 = fragCoord;
    float param_49 = pixel;
    float2 param_50 = light;
    coverage = fast::max(coverage, wheel(param_45, param_46, param_47, param_48, param_49, param_50, _76));
    int param_51 = 6;
    float param_52 = angleG;
    float2 param_53 = uv;
    float2 param_54 = fragCoord;
    float param_55 = pixel;
    float2 param_56 = light;
    coverage = fast::max(coverage, wheel(param_51, param_52, param_53, param_54, param_55, param_56, _76));
...
fragment main0_out main0(constant Globals& _76 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _76);
    out._fragColor = param;
    return out;
}
"""#
...
"""#
    ...
    public static let all: [ShaderProgram] = [apollonian, aurora, chladni, contour, gears, gradient, hyperspace, matrix, mode7, moire, mystify, raindrops, raster, saturn, starwars, synthwave, toasters, tunnel]
}
