...
// shaders-sha256: a34ed6e7f06f2f9ea0daf2350979a8f598d620fb32fbeae69fc4d1131fa08e9e
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 7> _493 = spvUnsafeArray<float4, 7>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(23.229999542236328125, 8.770000457763671875, 2.0, 0.449999988079071044921875), float4(38.0, 20.8899993896484375, 2.0, 0.449999988079071044921875), float4(132.25, 3.0, 0.60000002384185791015625, 1.0), float4(56.8899993896484375, 22.0, 2.5, -0.300000011920928955078125), float4(135.25, 12.0, 1.5, -0.300000011920928955078125) });
constant spvUnsafeArray<float4, 1> _697 = spvUnsafeArray<float4, 1>({ float4(6.0, 5.0, 40.0, 6.5) });
constant spvUnsafeArray<float2, 31> _866 = spvUnsafeArray<float2, 31>({ float2(1.0, 0.0), float2(1.0), float2(1.0, 2.0), float2(2.0), float2(3.0, 2.0), float2(4.0, 2.0), float2(4.0, 3.0), float2(4.0), float2(4.0, 5.0), float2(4.0, 6.0), float2(4.0, 7.0), float2(4.0, 8.0), float2(4.0, 9.0), float2(4.0, 10.0), float2(3.0, 10.0), float2(2.0, 10.0), float2(1.0, 10.0), float2(0.0, 10.0), float2(-1.0, 10.0), float2(-2.0, 10.0), float2(-3.0, 10.0), float2(-3.0, 9.0), float2(-3.0, 8.0), float2(-3.0, 7.0), float2(-3.0, 6.0), float2(-2.0, 6.0), float2(-1.0, 6.0), float2(0.0, 6.0), float2(1.0, 6.0), float2(1.0, 7.0), float2(1.0, 8.0) });
constant spvUnsafeArray<int, 64> _1175 = spvUnsafeArray<int, 64>({ 0, 10, 0, 0, 10, 10, 0, 0, 0, 10, 0, 0, 10, 10, 0, 0, 5, 13, 5, 5, 14, 9, 5, 5, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 5, 6, 0, 0, 10, 3, 5, 5, 0, 10, 0, 0, 10, 10, 0, 0 });
...
float2 waypoint(thread const float& k)
{
    ...
    return _866[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1272 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1272 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1283 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1283 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1272, _1283);
    float3 _1369;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1369 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1369 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1369;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1175[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1416 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1416 = float2(0.0);
        }
        float2 owner = cell - _1416;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1587 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1587 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1587;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1611 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1611;
    ...
    if (rd.x == 0.0)
    {
        _1655 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1655 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1665 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1665 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1655, _1665);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1719 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1719;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1734 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1734;
...
{
    float2 param = cell;
    bool _1837 = !inBlackout(param);
    bool _1845;
    if (_1837)
    {
        float2 param_1 = cell;
        _1845 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1845 = _1837;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2172 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3284)
{
    float2 res = float2(_3284.iResolution[0], _3284.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3284.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3284.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_14;
    float3 param_15;
    float _3598 = trace(param_12, param_13, param_14, param_15);
    int id = param_14;
    float3 n = param_15;
    float dist = _3598;
    float3 p = ro + (rd * dist);
    ...
    float3 param_21;
    float3 _3632 = surface(param_16, param_17, param_18, param_19, param_20, param_21);
    ...
    float2 param_27 = (chromaAt + fseed.yx) + float2(7.0);
    float2 blotch = (float2(_noise(param_26), _noise(param_27)) * 2.0) - float2(1.0);
...
fragment main0_out main0(constant Globals& _3284 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3284);
    out._fragColor = param;
    return out;
}
"""#
