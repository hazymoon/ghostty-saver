...
// shaders-sha256: 10f0769cb4db2362de133d1ccb35bd87dde47504cab284ed49fd1e57cf3fd174
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 7> _510 = spvUnsafeArray<float4, 7>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(23.229999542236328125, 8.770000457763671875, 2.0, 0.449999988079071044921875), float4(38.0, 20.8899993896484375, 2.0, 0.449999988079071044921875), float4(132.25, 3.0, 0.60000002384185791015625, 1.0), float4(56.8899993896484375, 22.0, 2.5, -0.300000011920928955078125), float4(135.25, 12.0, 1.5, -0.300000011920928955078125) });
constant spvUnsafeArray<float4, 1> _714 = spvUnsafeArray<float4, 1>({ float4(6.0, 5.0, 40.0, 6.5) });
constant spvUnsafeArray<float2, 31> _883 = spvUnsafeArray<float2, 31>({ float2(1.0, 0.0), float2(1.0), float2(1.0, 2.0), float2(2.0), float2(3.0, 2.0), float2(4.0, 2.0), float2(4.0, 3.0), float2(4.0), float2(4.0, 5.0), float2(4.0, 6.0), float2(4.0, 7.0), float2(4.0, 8.0), float2(4.0, 9.0), float2(4.0, 10.0), float2(3.0, 10.0), float2(2.0, 10.0), float2(1.0, 10.0), float2(0.0, 10.0), float2(-1.0, 10.0), float2(-2.0, 10.0), float2(-3.0, 10.0), float2(-3.0, 9.0), float2(-3.0, 8.0), float2(-3.0, 7.0), float2(-3.0, 6.0), float2(-2.0, 6.0), float2(-1.0, 6.0), float2(0.0, 6.0), float2(1.0, 6.0), float2(1.0, 7.0), float2(1.0, 8.0) });
constant spvUnsafeArray<int, 64> _1192 = spvUnsafeArray<int, 64>({ 0, 10, 0, 0, 10, 10, 0, 0, 0, 10, 0, 0, 10, 10, 0, 0, 5, 13, 5, 5, 14, 9, 5, 5, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 5, 6, 0, 0, 10, 3, 5, 5, 0, 10, 0, 0, 10, 10, 0, 0 });
...
float2 waypoint(thread const float& k)
{
    ...
    return _883[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1289 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1289 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1300 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1300 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1289, _1300);
    float3 _1386;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1386 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1386 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1386;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1192[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
static inline __attribute__((always_inline))
float wallTop(thread const float2& owner, thread const bool& axisX)
{
    float2 c = mod(owner, float2(8.0));
    float2 param = float2((c.x * 2.0) + float(axisX), c.y) + float2(0.0, 48.0);
    float h = cheap21(param);
    if (h > 0.07999999821186065673828125)
    {
        return 2.7000000476837158203125;
    }
    return 1.85000002384185791015625 + (0.3499999940395355224609375 * (h / 0.07999999821186065673828125));
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1466 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1466 = float2(0.0);
        }
        float2 owner = cell - _1466;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            float2 param_2 = owner;
            bool param_3 = axisX;
            if ((ro.y + (rd.y * enter)) > wallTop(param_2, param_3))
            {
                continue;
            }
            best = enter;
            ...
            if (onFace == axisX)
            {
                _1653 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1653 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1653;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1677 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1677;
    ...
    if (rd.x == 0.0)
    {
        _1720 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1720 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1730 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1730 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1720, _1730);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1784 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1784;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1799 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1799;
...
{
    float2 param = cell;
    bool _1902 = !inBlackout(param);
    bool _1910;
    if (_1902)
    {
        float2 param_1 = cell;
        _1910 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1910 = _1902;
...
static inline __attribute__((always_inline))
float boxMask(thread const float2& q, thread const float2& rad)
{
    float2 d = abs(q) - rad;
    return step(fast::max(d.x, d.y), 0.0);
}

static inline __attribute__((always_inline))
float3 wallDecal(thread const float2& owner, thread const bool& axisX, thread const float& along, thread const float& v, thread const float3& base)
{
    float2 c = mod(owner, float2(8.0));
    float2 key = float2((c.x * 2.0) + float(axisX), c.y);
    float2 param = key + float2(0.0, 96.0);
    float h = cheap21(param);
    if (h > 0.1500000059604644775390625)
    {
        return base;
    }
    int kind = int((h / 0.1500000059604644775390625) * 4.0);
    float2 param_1 = key + float2(0.0, 112.0);
    float g = cheap21(param_1);
    float2 param_2 = owner;
    bool param_3 = axisX;
    int part = wallOn(param_2, param_3);
    float _2372;
    if (part == 2)
    {
        _2372 = 1.0;
    ...
    {
        _2372 = (part == 3) ? 3.0 : 2.0;
    }
    float cx = _2372;
    if (kind == 0)
    {
        float2 rad = float2(0.1500000059604644775390625 + (0.0500000007450580596923828125 * g), 0.25 + (0.100000001490116119384765625 * fract(g * 37.0)));
        float2 q = float2(along - cx, v - (2.599999904632568359375 - rad.y));
        float2 param_4 = q;
        float2 param_5 = rad;
        float panel = boxMask(param_4, param_5);
        float2 param_6 = q;
        float2 param_7 = rad + float2(0.04500000178813934326171875);
        float frame = boxMask(param_6, param_7) - panel;
        float knob = step(length(q - float2(rad.x * 0.60000002384185791015625, 0.0)), 0.02500000037252902984619140625) * panel;
        float3 col = mix(base, float3(0.2599999904632568359375, 0.17000000178813934326171875, 0.0900000035762786865234375), float3(panel));
        col = mix(col, float3(0.11699999868869781494140625, 0.076499998569488525390625, 0.04050000011920928955078125), float3(frame));
        return mix(col, float3(0.519999980926513671875, 0.439999997615814208984375, 0.23999999463558197021484375), float3(knob));
    }
    if (kind == 1)
    {
        float2 rad_1 = float2(0.449999988079071044921875, 1.0);
        float2 q_1 = float2(along - cx, v - rad_1.y);
        float2 param_8 = q_1;
        float2 param_9 = rad_1;
        float panel_1 = boxMask(param_8, param_9);
        float2 param_10 = q_1;
        float2 param_11 = rad_1 + float2(0.0500000007450580596923828125);
        float frame_1 = boxMask(param_10, param_11) - panel_1;
        bool block = fract(g * 149.0) < 0.5;
        float2 _2502;
        if (block)
        {
            _2502 = float2(3.0, 2.0 + floor(fract(g * 37.0) * 2.0));
        }
        else
        {
            _2502 = float2(1.0, 5.0 + floor(fract(g * 37.0) * 5.0));
        }
        float2 count = _2502;
        float2 pitch = float2(0.2599999904632568359375, 0.180000007152557373046875);
        float2 at = float2(block ? 0.0 : 0.2599999904632568359375, 0.0);
        float2 lim = (count - float2(1.0)) * 0.5;
        float2 gi = fast::clamp(round((q_1 - at) / pitch), -lim, lim);
        float knob_1 = step(length((q_1 - at) - (gi * pitch)), 0.032999999821186065673828125) * panel_1;
        float3 col_1 = mix(base, float3(0.2599999904632568359375, 0.17000000178813934326171875, 0.0900000035762786865234375), float3(panel_1));
        col_1 = mix(col_1, float3(0.11699999868869781494140625, 0.076499998569488525390625, 0.04050000011920928955078125), float3(frame_1));
        return mix(col_1, float3(0.519999980926513671875, 0.439999997615814208984375, 0.23999999463558197021484375), float3(knob_1));
    }
    if (kind == 2)
    {
        float rad_2 = 0.25;
        float2 q_2 = float2(along - cx, v - (1.14999997615814208984375 + (0.25 * g)));
        float2 param_12 = q_2;
        float2 param_13 = float2(rad_2);
        float hole = boxMask(param_12, param_13);
        float2 param_14 = q_2;
        float2 param_15 = float2(rad_2 + 0.0350000001490116119384765625);
        float rim = boxMask(param_14, param_15) - hole;
        float slat = step(0.449999988079071044921875, fract((q_2.y + rad_2) * 9.0));
        float3 col_2 = mix(base, mix(float3(0.014999999664723873138427734375, 0.013500000350177288055419921875, 0.0089999996125698089599609375), float3(0.0500000007450580596923828125, 0.04500000178813934326171875, 0.02999999932944774627685546875), float3(slat)), float3(hole));
        return mix(col_2, base * 0.5, float3(rim));
    }
    float2 rad_3 = float2(0.449999988079071044921875, 1.0);
    float2 q_3 = float2(along - cx, v - rad_3.y);
    float2 param_16 = q_3;
    float2 param_17 = rad_3;
    if (boxMask(param_16, param_17) < 0.5)
    {
        return base;
    }
    float2 e2 = abs(q_3) / rad_3;
    float e = fast::max(fast::max(e2.x, e2.y), 0.001000000047497451305389404296875);
    float k = log(e) / (-0.544727146625518798828125);
    float ring_1 = (1.0 - smoothstep(0.0500000007450580596923828125, 0.119999997317790985107421875, abs(fract(k + 0.5) - 0.5))) * step(k, 4.5);
    float diag = 1.0 - smoothstep(0.0199999995529651641845703125, 0.0599999986588954925537109375, abs(e2.x - e2.y) / e);
    float3 col_3 = mix(float3(0.0500000007450580596923828125, 0.04500000178813934326171875, 0.02999999932944774627685546875), float3(0.4289999902248382568359375, 0.3630000054836273193359375, 0.16500000655651092529296875), float3(e));
    return mix(col_3, col_3 * 0.3499999940395355224609375, float3(fast::max(ring_1, diag)));
}
...
float3 surface(thread const float3& p, thread const int& id, thread const float3& n, thread const float& t, thread float3& emission)
{
    ...
    float3 paper = (((float3(0.7799999713897705078125, 0.660000026226043701171875, 0.300000011920928955078125) * (0.920000016689300537109375 + (0.07999999821186065673828125 * stripe))) * (0.800000011920928955078125 + (0.3499999940395355224609375 * grime))) * skirting) * top;
    if (id != 2)
    {
        return paper;
    }
    ...
    float across = _2921;
    float line = round(across / 4.0);
    if (abs(abs(across - (line * 4.0)) - 0.0599999986588954925537109375) > 0.02999999932944774627685546875)
    {
        return paper;
    }
    float seg = floor(u / 4.0);
    float2 _2955;
    if (abs(n.x) > 0.5)
    {
        _2955 = float2(line - 1.0, seg);
    }
    else
    {
        _2955 = float2(seg, line - 1.0);
    }
    float2 owner = _2955;
    float2 param_9 = owner;
    bool param_10 = abs(n.x) > 0.5;
    float param_11 = u - (seg * 4.0);
    float param_12 = p.y;
    float3 param_13 = paper;
    return wallDecal(param_9, param_10, param_11, param_12, param_13);
}
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2229 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3783)
{
    float2 res = float2(_3783.iResolution[0], _3783.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3783.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3783.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_14;
    float3 param_15;
    float _4096 = trace(param_12, param_13, param_14, param_15);
    int id = param_14;
    float3 n = param_15;
    float dist = _4096;
    float3 p = ro + (rd * dist);
    ...
    float param_19 = tNow;
    float3 param_20;
    float3 _4116 = surface(param_16, param_17, param_18, param_19, param_20);
    ...
    C += (blotch * ((0.039999999105930328369140625 * (1.0 + (storm * 1.0))) + (bandEdge * 0.3499999940395355224609375)));
    ...
    Y += ((((grain - 0.5) * 0.0599999986588954925537109375) * (1.0 + (storm * 1.0))) + (snow * ((0.017999999225139617919921875 * (1.0 + (storm * 1.0))) + (band * 0.550000011920928955078125))));
...
fragment main0_out main0(constant Globals& _3783 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3783);
    out._fragColor = param;
    return out;
}
"""#
