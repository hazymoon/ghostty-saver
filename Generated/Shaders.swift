...
// shaders-sha256: 27415395c1707bf22eaa2df0e2f5f57393719361532fe20937319fed7412eabb
// prefix-ref: v1.3.1
...
float walkSpeed()
{
    return 0.226415097713470458984375;
}
...
float pace(thread const float& u)
{
    ...
    float p2 = smoothstep(113.0, 115.0, u) - smoothstep(121.0, 123.0, u);
    return (1.0 - p1) - p2;
}
...
{
    float lap = floor(k / 36.0);
    return _396[int(k - (lap * 36.0))] + float2(0.0, lap * 8.0);
}

static inline __attribute__((always_inline))
float2 segmentIntegral(thread const float& k, thread const float& f0, thread const float& f1)
{
    float param = k;
    float2 a = waypoint(param);
    float param_1 = k + 1.0;
    float2 b = waypoint(param_1);
    return (a * (f1 - f0)) + (((b - a) * 0.5) * ((f1 * f1) - (f0 * f0)));
}

static inline __attribute__((always_inline))
float2 pathIntegral(thread const float& a, thread const float& b)
{
    float k = floor(a);
    if (b <= (k + 1.0))
    {
        ...
        float param_1 = a - k;
        float param_2 = b - k;
        return segmentIntegral(param, param_1, param_2);
    }
    float param_3 = k;
    float param_4 = a - k;
    float param_5 = 1.0;
    float param_6 = k + 1.0;
    float param_7 = 0.0;
    float param_8 = (b - k) - 1.0;
    return segmentIntegral(param_3, param_4, param_5) + segmentIntegral(param_6, param_7, param_8);
}

static inline __attribute__((always_inline))
float2 bodyAt(thread const float& s)
{
    float2 p = float2(0.0);
    for (int k = -2; k <= 2; k++)
    {
        float x = s + ((float(k) * 1.7999999523162841796875) * 0.20000000298023223876953125);
        float param = x - 0.180000007152557373046875;
        float param_1 = x + 0.180000007152557373046875;
        p += (pathIntegral(param, param_1) * (3.0 - abs(float(k))));
    }
    return p / float2(3.2400000095367431640625);
}
...
float2 onPath(thread const float& s)
{
    ...
    float param = k;
    float param_1 = k + 1.0;
    return mix(waypoint(param), waypoint(param_1), float2(s - k));
}

static inline __attribute__((always_inline))
float2 tangentAt(thread const float& s, thread const float& ahead)
{
    float2 dir = float2(0.0);
    for (int k = -2; k <= 2; k++)
    {
        float x = (s + ahead) + ((float(k) * 1.7999999523162841796875) * 0.20000000298023223876953125);
        float param = x + 0.180000007152557373046875;
        float param_1 = x - 0.180000007152557373046875;
        dir += ((onPath(param) - onPath(param_1)) * (3.0 - abs(float(k))));
    }
    return dir / float2(3.2400000095367431640625);
}

static inline __attribute__((always_inline))
float headingAt(thread const float& s, thread const float& ahead)
{
    float param = s;
    float param_1 = ahead;
    float2 dir = tangentAt(param, param_1);
    return precise::atan2(dir.x, dir.y);
}

static inline __attribute__((always_inline))
float turnRate(thread const float& s)
{
    float param = s + 0.0500000007450580596923828125;
    float param_1 = 0.0;
    float param_2 = s - 0.0500000007450580596923828125;
    float param_3 = 0.0;
    float d = headingAt(param, param_1) - headingAt(param_2, param_3);
    return precise::atan2(sin(d), cos(d)) / 0.100000001490116119384765625;
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _829 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _829 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _841 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _841 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_829, _841);
    float3 _926;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _926 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _926 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _926;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _724[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _950 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _950;
    ...
    if (rd.x == 0.0)
    {
        _994 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _994 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1004 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1004 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_994, _1004);
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1049 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1049;
        ...
        if (axisX)
        {
            ...
            {
                _1100 = cell - float2(1.0, 0.0);
            }
            float2 wallCell = _1100;
        ...
        else
        {
            ...
            {
                _1123 = cell - float2(0.0, 1.0);
            }
            float2 wallCell_1 = _1123;
        ...
        if (wallCovers(param_8, param_9))
        {
            ...
            if (axisX)
            {
                _1148 = float3(-_step.x, 0.0, 0.0);
            }
            else
            {
                _1148 = float3(0.0, 0.0, -_step.y);
            }
            normal = _1148;
            return exit;
        }
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _1760)
{
    float param = _1760.iTime;
    float t = droppedFrames(param);
    float2 res = float2(_1760.iResolution[0], _1760.iResolution[1]);
    float mask = 1.0;
    ...
    float param_2 = _1760.iTime;
    float band = trackingBand(param_1, param_2);
    float param_3 = floor(screenY * 90.0) + floor(_1760.iTime * 30.0);
    ...
    float param_4 = floor(_1760.iTime * 60.0);
    float headShift = (headSwitch * 0.02999999932944774627685546875) * (hash11(param_4) - 0.5);
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_1760.iTime * 21.0))) * (0.5 + (0.5 * sin(_1760.iTime * 0.699999988079071044921875)));
    uv.x += ((bandShift + headShift) + wobble);
    float r2 = dot(uv, uv);
    uv *= (1.0 + (0.0500000007450580596923828125 * r2));
    float u = mod(t, 175.0);
    float param_5 = u;
    float s = (floor(t / 175.0) * 36.0) + walked(param_5);
    float param_6 = u;
    float moving = pace(param_6);
    float param_7 = t * 0.17000000178813934326171875;
    float steps = (s * 6.80000019073486328125) + (0.2199999988079071044921875 * fbm1(param_7));
    ...
    float param_8 = stepNo;
    float param_9 = stepNo + 1.0;
    float amp = mix(stepSize(param_8), stepSize(param_9), su) * moving;
    float param_10 = s;
    float2 body = bodyAt(param_10);
    float param_11 = s;
    float param_12 = 0.0;
    float heading = headingAt(param_11, param_12);
    float param_13 = s;
    float turning = fast::clamp(turnRate(param_13) / 1.5, -1.0, 1.0);
    float warped = s - (((moving * 0.64999997615814208984375) / 21.3628292083740234375) * cos(6.283185482025146484375 * strideP));
    float param_14 = warped;
    float param_15 = 0.25;
    float neck = headingAt(param_14, param_15) - heading;
    neck = fast::clamp(precise::atan2(sin(neck), cos(neck)), -0.730000019073486328125, 0.730000019073486328125);
    float param_16 = u;
    float param_17 = 30.0;
    float param_18 = u;
    float param_19 = 113.0;
    ...
    float param_20 = sinceStrike;
    float param_21 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + ((0.0040000001899898052215576171875 * moving) * ring(param_20, param_21));
    ...
    float surge = ((((0.0500000007450580596923828125 * moving) * walkSpeed()) * 4.0) / (6.283185482025146484375 * stepHz)) * sin(6.283185482025146484375 * su);
    float roll = (0.007000000216066837310791015625 * moving) * sin(6.283185482025146484375 * strideP);
    float param_22 = sinceStrike;
    float param_23 = 0.07999999821186065673828125;
    float kick = (0.001700000022538006305694580078125 * moving) * ring(param_22, param_23);
    ...
    int param_26;
    float3 param_27;
    float _2246 = trace(param_24, param_25, param_26, param_27);
    int id = param_26;
    float3 n = param_27;
    float dist = _2246;
    float3 p = ro + (rd * dist);
    ...
    float param_31 = t;
    float3 param_32;
    float3 _2266 = surface(param_28, param_29, param_30, param_31, param_32);
    float3 emission = param_32;
    float3 albedo = _2266;
    float3 param_33 = p;
    float3 param_34 = n;
    float param_35 = t;
    float3 col = (albedo * (lighting(param_33, param_34, param_35) + float3(0.01200000010430812835693359375))) + emission;
    ...
    float luma = dot(col, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float3 chroma = col - float3(luma);
    float3 dchroma = dcol - float3(dot(dcol, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)));
    col = (float3(luma) + chroma) + (dchroma * 6.0);
    ...
    float2 param_36 = floor(fragCoord / float2(1.5)) + float2(fract(_1760.iTime * 7.0) * 100.0);
    float grain = hash21(param_36);
    col += float3((grain - 0.5) * 0.0900000035762786865234375);
    float2 param_37 = float2(floor(screenY * 240.0), floor(_1760.iTime * 30.0));
    float bandNoise = hash21(param_37);
    ...
    float2 param_38 = fragCoord + float2(fract(_1760.iTime * 13.0) * 50.0);
    float snow = hash21(param_38);
...
fragment main0_out main0(constant Globals& _1760 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _1760);
    out._fragColor = param;
    return out;
}
"""#
