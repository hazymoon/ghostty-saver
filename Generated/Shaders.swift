...
// shaders-sha256: 8731da349478250c5b1036036bde05b14514b0274f22d58c989f99fddc402d99
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 7> _591 = spvUnsafeArray<float4, 7>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(23.229999542236328125, 8.770000457763671875, 2.0, 0.449999988079071044921875), float4(38.0, 20.8899993896484375, 2.0, 0.449999988079071044921875), float4(126.0, 9.0, 0.60000002384185791015625, 1.0), float4(56.8899993896484375, 22.0, 2.5, -0.300000011920928955078125), float4(135.0, 12.0, 1.5, -0.300000011920928955078125) });
constant spvUnsafeArray<float4, 2> _798 = spvUnsafeArray<float4, 2>({ float4(6.0, 5.0, 40.0, 6.5), float4(3.0, 6.0, 124.8000030517578125, 8.0) });
constant spvUnsafeArray<float2, 31> _966 = spvUnsafeArray<float2, 31>({ float2(1.0, 0.0), float2(1.0), float2(1.0, 2.0), float2(2.0), float2(3.0, 2.0), float2(4.0, 2.0), float2(4.0, 3.0), float2(4.0), float2(4.0, 5.0), float2(4.0, 6.0), float2(4.0, 7.0), float2(4.0, 8.0), float2(4.0, 9.0), float2(4.0, 10.0), float2(3.0, 10.0), float2(2.0, 10.0), float2(1.0, 10.0), float2(0.0, 10.0), float2(-1.0, 10.0), float2(-2.0, 10.0), float2(-3.0, 10.0), float2(-3.0, 9.0), float2(-3.0, 8.0), float2(-3.0, 7.0), float2(-3.0, 6.0), float2(-2.0, 6.0), float2(-1.0, 6.0), float2(0.0, 6.0), float2(1.0, 6.0), float2(1.0, 7.0), float2(1.0, 8.0) });
constant spvUnsafeArray<int, 64> _1274 = spvUnsafeArray<int, 64>({ 0, 10, 0, 0, 10, 10, 0, 0, 0, 10, 0, 0, 10, 10, 0, 0, 5, 13, 5, 5, 14, 9, 5, 5, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 5, 6, 0, 0, 10, 3, 5, 5, 0, 10, 0, 0, 10, 10, 0, 0 });
...
float stormAt(thread const float& u)
{
    ...
    float param_5 = u;
    float param_6 = 120.0;
    float param_7 = 123.90000152587890625;
    float param_8 = 129.5;
    float param_9 = 136.0;
...
static inline __attribute__((always_inline))
float agcAt(thread const float& u)
{
    float param = u;
    float param_1 = 125.1999969482421875;
    float param_2 = 126.09999847412109375;
    float param_3 = 132.3000030517578125;
    float param_4 = 132.899993896484375;
...
static inline __attribute__((always_inline))
float zoomAt(thread const float& u)
{
    float param = u;
    float param_1 = 126.40000152587890625;
    float param_2 = 128.600006103515625;
    float param_3 = 132.3000030517578125;
    float param_4 = 133.600006103515625;
    float on = bump(param, param_1, param_2, param_3, param_4);
    float param_5 = u;
    float param_6 = 128.0500030517578125;
    float param_7 = 128.600006103515625;
    float param_8 = 128.720001220703125;
    float param_9 = 129.0;
    float over = 0.0500000007450580596923828125 * bump(param_5, param_6, param_7, param_8, param_9);
    return 1.0 + (1.5 * (on + over));
}
...
float2 waypoint(thread const float& k)
{
    ...
    return _966[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
void smoothPath(thread const float& s, thread float2& pos, thread float2& dir, thread float2& turn)
{
    ...
    for (float k = k0; k <= k1; k += 1.0)
    {
        ...
        for (int part = 0; part < 2; part++)
        {
            ...
            {
                _1045 = fast::max(a, 0.0);
            }
            float ta = _1045;
            if (part == 0)
            {
                _1056 = fast::min(b, 0.0);
            }
            else
            {
                _1056 = b;
...
Pose cameraPose(thread const float& t)
{
    ...
    float2 tileEye = ((body - float2(0.0, floor(t / 150.0) * 8.0)) + float2(0.5)) * 4.0;
    float param_29 = u;
    float2 param_30 = tileEye;
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1371 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1371 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1382 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1382 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1371, _1382);
    float3 _1468;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1468 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1468 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1468;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1274[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1515 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1515 = float2(0.0);
        }
        float2 owner = cell - _1515;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1686 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1686 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1686;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1710 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1710;
    ...
    if (rd.x == 0.0)
    {
        _1754 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1754 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1764 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1764 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1754, _1764);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1818 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1818;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1833 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1833;
...
{
    float2 param = cell;
    bool _1936 = !inBlackout(param);
    bool _1944;
    if (_1936)
    {
        float2 param_1 = cell;
        _1944 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1944 = _1936;
...
static inline __attribute__((always_inline))
float tubeMains(thread const float& u, thread const float& h)
{
    if ((u < 124.8000030517578125) || (u > 136.4499969482421875))
    {
        return 1.0;
    }
    float back = 132.3000030517578125 + (3.0 * fract(h * 7.30000019073486328125));
    float lit = smoothstep(back, back + 0.449999988079071044921875, u);
    float gutter = step(0.300000011920928955078125, (fract((u * 11.0) + (h * 53.0)) * fract((u * 7.0) + (h * 131.0))) * 3.0);
    lit = mix(lit * gutter, lit, smoothstep(back + 0.449999988079071044921875, (back + 0.449999988079071044921875) + 0.60000002384185791015625, u));
    return mix(1.0, lit, smoothstep(124.8000030517578125, 124.8600006103515625, u));
}
...
float tubeLevel(thread const float2& cell, thread const float& t)
{
    ...
    float kept = float(all(mod(cell, float2(8.0)) == float2(3.0, 6.0)));
    float param_2 = mod(t, 150.0);
    float param_3 = h;
    float power = fast::max(kept, tubeMains(param_2, param_3));
    if (power == 0.0)
    {
        return 0.0;
    }
    ...
    float param_4 = (slot * 3.7000000476837158203125) + seed;
    if (hash11(param_4) > 0.1500000059604644775390625)
    {
        return 1.0;
    }
    float param_5 = ((slot * 1.2999999523162841796875) + seed) + 4.0;
    float start = hash11(param_5) * 9.30000019073486328125;
    float within = tt - (slot * 10.0);
    float fit = step(start, within) * step(within, start + 0.699999988079071044921875);
    float param_6 = floor(tt * 16.0) + seed;
    float drop = step(0.550000011920928955078125, hash11(param_6));
    return power * mix(1.0, 0.300000011920928955078125, fit * drop);
}

static inline __attribute__((always_inline))
float3 surface(thread const float3& p, thread const int& id, thread const float3& n, thread const float& t, thread const float& footprint, thread float3& emission)
{
    emission = float3(0.0);
    if (id == 0)
    {
    ...
    if (id == 1)
    {
        ...
        float aa = footprint / 0.60000002384185791015625;
        float hw = 0.0199999995529651641845703125;
        float draw = fast::max(hw, aa);
        float toBar = 0.5 - fast::max(tile.x, tile.y);
        ...
        float on = fast::min(lit / 0.300000011920928955078125, 1.0);
        float2 param_7 = p.xz * 6.0;
        float3 albedo = (float3(0.699999988079071044921875, 0.660000026226043701171875, 0.5) * (0.89999997615814208984375 + (0.20000000298023223876953125 * _noise(param_7)))) * (1.0 - (0.300000011920928955078125 * seam));
        albedo = mix(albedo, float3(0.25), float3(panel * (1.0 - (level0 * on))));
        emission = ((float3(1.0, 0.980000019073486328125, 0.819999992847442626953125) * panel) * level0) * ((0.60000002384185791015625 * on) + (1.39999997615814208984375 * lit));
        return mix(albedo, float3(0.0), float3((panel * level0) * on));
    }
    float _2673;
    if (abs(n.x) > 0.5)
    {
...
float3 lighting(thread const float3& p, thread const float3& n, thread const float& t)
{
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            float3 l = lp - p;
            float d2 = dot(l, l);
            float window = 1.0 - smoothstep(18.0, 36.0, d2);
            bounced += (((level0 * 1.0) * window) / (1.0 + (d2 * 0.300000011920928955078125)));
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2368 / 4.0);
                if (wallCovers(param_12, param_13))
                {
                    continue;
                }
            }
            direct += ((((level0 * lambert) * 2.900000095367431640625) * window) / (1.0 + (d2 * 0.20000000298023223876953125)));
        }
    }
...
static inline __attribute__((always_inline))
float figureShows(thread const float& u)
{
    float param = u;
    float param_1 = 124.8000030517578125;
    float param_2 = 125.1999969482421875;
    float param_3 = 132.3000030517578125;
    float param_4 = 132.4499969482421875;
    return bump(param, param_1, param_2, param_3, param_4);
}

static inline __attribute__((always_inline))
float picked(thread const float& i, thread const float& a, thread const float& b, thread const float& two)
{
    return fast::max(step(abs(i - a), 0.25), two * step(abs(i - b), 0.25));
}

static inline __attribute__((always_inline))
Wrong wrongOf(thread const float& lap)
{
    float2 param = float2(lap, 3.0);
    float a = floor(cheap21(param) * 6.0);
    float2 param_1 = float2(lap, 17.0);
    float b = floor(cheap21(param_1) * 6.0);
    float2 param_2 = float2(lap, 29.0);
    float two = step(0.5, cheap21(param_2));
    float param_3 = 0.0;
    float param_4 = a;
    float param_5 = b;
    float param_6 = two;
    ...
    float param_25 = b;
    float param_26 = two;
    return Wrong{ picked(param_3, param_4, param_5, param_6), picked(param_7, param_8, param_9, param_10), picked(param_11, param_12, param_13, param_14), picked(param_15, param_16, param_17, param_18), picked(param_19, param_20, param_21, param_22), picked(param_23, param_24, param_25, param_26) };
}

static inline __attribute__((always_inline))
float segment(thread const float2& p, thread const float2& a, thread const float2& b)
{
    float2 pa = p - a;
    float2 ba = b - a;
    return length(pa - (ba * fast::clamp(dot(pa, ba) / dot(ba, ba), 0.0, 1.0)));
}

static inline __attribute__((always_inline))
float figureDist(thread float2& p, thread const Wrong& w)
{
    p.y -= (0.119999997317790985107421875 * w.hover);
    float scale = 1.0 + (0.3499999940395355224609375 * w.tall);
    p /= float2(scale);
    float lift = 0.100000001490116119384765625 * w.tilt;
    float2 shoulderL = float2(-0.189999997615814208984375, 1.41999995708465576171875 + lift);
    float2 shoulderR = float2(0.189999997615814208984375, 1.41999995708465576171875);
    float2 param = p;
    float2 param_1 = float2(-0.100000001490116119384765625, 0.920000016689300537109375);
    float2 param_2 = float2(-0.10999999940395355224609375, 0.0);
    float d = segment(param, param_1, param_2) - 0.07500000298023223876953125;
    float2 param_3 = p;
    float2 param_4 = float2(0.100000001490116119384765625, 0.920000016689300537109375);
    float2 param_5 = float2(0.10999999940395355224609375, 0.0);
    d = fast::min(d, segment(param_3, param_4, param_5) - 0.07500000298023223876953125);
    float2 param_6 = p;
    float2 param_7 = float2(0.0, 0.939999997615814208984375);
    float2 param_8 = float2(0.0, 1.39999997615814208984375);
    d = fast::min(d, segment(param_6, param_7, param_8) - 0.1500000059604644775390625);
    float neckTop = 1.5 + (0.1599999964237213134765625 * w.neck);
    float2 param_9 = p;
    float2 param_10 = float2(0.0, 1.39999997615814208984375);
    float2 param_11 = float2(0.0, neckTop);
    d = fast::min(d, segment(param_9, param_10, param_11) - 0.0500000007450580596923828125);
    d = fast::min(d, length(p - float2(0.0, neckTop + 0.104999996721744537353515625)) - 0.104999996721744537353515625);
    float2 elbowR = float2(0.23499999940395355224609375, 1.10000002384185791015625 - (0.20000000298023223876953125 * w.arm));
    float2 handR = float2(0.2150000035762786865234375, 0.800000011920928955078125 - (0.449999988079071044921875 * w.arm));
    float2 elbowL = float2((-0.23499999940395355224609375) + (0.189999997615814208984375 * w.elbow), 1.10000002384185791015625);
    float2 param_12 = p;
    float2 param_13 = shoulderR;
    float2 param_14 = elbowR;
    d = fast::min(d, segment(param_12, param_13, param_14) - 0.0500000007450580596923828125);
    float2 param_15 = p;
    float2 param_16 = elbowR;
    float2 param_17 = handR;
    d = fast::min(d, segment(param_15, param_16, param_17) - 0.0500000007450580596923828125);
    float2 param_18 = p;
    float2 param_19 = shoulderL;
    float2 param_20 = elbowL;
    d = fast::min(d, segment(param_18, param_19, param_20) - 0.0500000007450580596923828125);
    float2 param_21 = p;
    float2 param_22 = elbowL;
    float2 param_23 = float2(-0.2150000035762786865234375, 0.800000011920928955078125);
    d = fast::min(d, segment(param_21, param_22, param_23) - 0.0500000007450580596923828125);
    return d * scale;
}

static inline __attribute__((always_inline))
float2 cheap22(thread const float2& p)
{
    uint2 v = (uint2(int2(p)) * uint2(1664525u)) + uint2(1013904223u);
    v.x += (v.y * 1664525u);
    v.y += (v.x * 1664525u);
    ...
    v.y += (v.x * 1664525u);
    v ^= (v >> uint2(16u));
    return float2(v & uint2(16777215u)) / float2(16777216.0);
}

static inline __attribute__((always_inline))
float2 _noise2(thread const float2& p)
{
    float2 i = floor(p);
    float2 f = fract(p);
    f = (f * f) * (float2(3.0) - (f * 2.0));
    ...
    float2 param_2 = i + float2(0.0, 1.0);
    float2 param_3 = i + float2(1.0);
    return mix(mix(cheap22(param), cheap22(param_1), float2(f.x)), mix(cheap22(param_2), cheap22(param_3), float2(f.x)), float2(f.y));
}

static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3847)
{
    float2 res = float2(_3847.iResolution[0], _3847.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3847.iTime, 3600.0) * 59.939998626708984375;
    ...
    float param_5 = lapNow;
    float gain = 1.0 + (2.0 * agcAt(param_5));
    float grainGain = 1.0 + ((gain - 1.0) * 0.3499999940395355224609375);
    float param_6 = screenY;
    float param_7 = now;
    float param_8 = storm;
    float band = trackingBand(param_6, param_7, param_8);
    float param_9 = floor(screenY * 90.0) + fseed.x;
    float bandShift = band * (0.0500000007450580596923828125 + (0.039999999105930328369140625 * hash11(param_9)));
    ...
    float param_10 = now;
    float param_11 = storm;
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (now * 21.0))) * timeBaseError(param_10, param_11);
    uv.x += ((bandShift + headShift) + wobble);
    float param_12 = lapNow;
    float zoom = zoomAt(param_12);
    float focal = 1.25 * zoom;
    float r2 = dot(uv, uv);
    uv *= (1.0 + ((0.0500000007450580596923828125 * r2) / zoom));
    float param_13 = tNow;
    Pose cam = cameraPose(param_13);
    ...
    if (zoom > 1.0)
    {
        float hand = (0.00179999996908009052276611328125 * (zoom - 1.0)) / 1.5;
        float param_14 = tNow * 2.7000000476837158203125;
        float param_15 = (tNow * 3.2999999523162841796875) + 41.0;
        ruv += (float2(fbm1(param_14), fbm1(param_15)) * (focal * hand));
    }
    float3 rd = fast::normalize(((cam.forward * focal) + (cam.right * ruv.x)) + (cam.up * ruv.y));
    rd += (float3(rd.z, 0.0, -rd.x) * (cam.yawRate * back));
    float3 param_16 = ro;
    float3 param_17 = rd;
    int param_18;
    float3 param_19;
    float _4213 = trace(param_16, param_17, param_18, param_19);
    int id = param_18;
    float3 n = param_19;
    float dist = _4213;
    float3 p = ro + (rd * dist);
    float footprint = (dist / (frame.y * 1.25)) / fast::max(abs(dot(n, rd)), 0.0500000007450580596923828125);
    float3 param_20 = p;
    int param_21 = id;
    float3 param_22 = n;
    float param_23 = tNow;
    float param_24 = footprint;
    float3 param_25;
    float3 _4247 = surface(param_20, param_21, param_22, param_23, param_24, param_25);
    float3 emission = param_25;
    float3 albedo = _4247;
    float3 param_26 = p;
    float3 param_27 = n;
    float param_28 = tNow;
    float3 col = (albedo * (lighting(param_26, param_27, param_28) + float3(0.02500000037252902984619140625))) + emission;
    ...
    float param_29 = lapNow;
    float shows = figureShows(param_29);
    if (shows > 0.0)
    {
        float param_30 = floor(tNow / 150.0);
        Wrong wrong = wrongOf(param_30);
        float3 foot = float3(14.0, 0.0, (26.0 + ((floor(tNow / 150.0) * 8.0) * 4.0)) + 1.10000002384185791015625);
        float3 face = fast::normalize(float3(ro.x - foot.x, 0.0, ro.z - foot.z));
        float3 across = float3(-face.z, 0.0, face.x);
        float tHit = dot(foot - ro, face) / fast::min(dot(rd, face), -9.9999997473787516355514526367188e-05);
        float3 hit = ro + (rd * tHit);
        float2 local = float2(dot(hit - foot, across), hit.y);
        float scale = 1.0 + (0.3499999940395355224609375 * wrong.tall);
        bool _4355 = (tHit > 0.0) && (tHit < dist);
        bool _4365;
        if (_4355)
        {
            _4365 = abs(local.x) < (0.4000000059604644775390625 * scale);
        }
        else
        {
            _4365 = _4355;
        ...
        if (_4365)
        {
            _4371 = local.y > (-0.100000001490116119384765625);
        }
        else
        {
            _4371 = _4365;
        ...
        if (_4371)
        {
            _4382 = local.y < (((1.75 * scale) + 0.119999997317790985107421875) + 0.100000001490116119384765625);
        }
        else
        {
            _4382 = _4371;
        ...
        if (_4382)
        {
            ...
            float2 param_31 = local;
            Wrong param_32 = wrong;
            float _4401 = figureDist(param_31, param_32);
            float cover = shows * smoothstep(aa, -aa, _4401);
            if (cover > 0.0)
            {
                float3 param_33 = hit;
                float3 param_34 = face;
                float param_35 = tNow;
                float3 lit = float3(0.0199999995529651641845703125) * (lighting(param_33, param_34, param_35) + float3(0.01200000010430812835693359375));
                col = mix(col, mix(float3(0.0900000035762786865234375, 0.07500000298023223876953125, 0.02999999932944774627685546875), lit, float3(exp((-tHit) * 0.07500000298023223876953125))), float3(cover));
            }
        }
    }
    ...
    float2 chromaAt = (inFrame / frame) * float2(40.0, 240.0);
    float bandEdge = (band * (1.0 - band)) * 4.0;
    C += (dyiq.yz * 6.0);
    float2 param_36 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_36) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_37 = chromaAt + fseed;
    float2 blotch = (_noise2(param_37) * 2.0) - float2(1.0);
    C *= (1.0 - (band * 0.85000002384185791015625));
    C += (blotch * (((0.039999999105930328369140625 * (1.0 + (storm * 1.0))) * grainGain) + (bandEdge * 0.3499999940395355224609375)));
    ...
    float2 param_38 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_38);
    float snowGate = 0.930000007152557373046875 - (storm * 0.119999997317790985107421875);
    float2 param_39 = (lumaCell + fseed) + float2(3.0);
    float2 param_40 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(snowGate, cheap21(param_39)) - step(snowGate, cheap21(param_40));
    Y += (((((grain - 0.5) * 0.0599999986588954925537109375) * (1.0 + (storm * 1.0))) * grainGain) + (snow * (((0.017999999225139617919921875 * (1.0 + (storm * 1.0))) * grainGain) + (band * 0.550000011920928955078125))));
    float2 param_41 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_41);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_42 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_42);
...
fragment main0_out main0(constant Globals& _3847 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3847);
    out._fragColor = param;
    return out;
}
"""#
