...
// shaders-sha256: 1e1bc750e6af7fdc4457f7a61621a49e3521a0af4f264b81795eed0bcfa0cf30
// prefix-ref: v1.3.1
...
float tubeLevel(thread const float2& cell, thread const float& t)
{
    ...
    {
        float _2043;
        if (all(mod(cell, float2(8.0)) == float2(3.0, 6.0)))
        {
            _2043 = 1.0;
        }
        else
        {
            float param_2 = u;
            float param_3 = h;
            _2043 = tubeMains(param_2, param_3);
        }
        power = _2043;
        if (power == 0.0)
        {
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2370 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3849)
{
    float2 res = float2(_3849.iResolution[0], _3849.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3849.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3849.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_18;
    float3 param_19;
    float _4215 = trace(param_16, param_17, param_18, param_19);
    int id = param_18;
    float3 n = param_19;
    float dist = _4215;
    float3 p = ro + (rd * dist);
    ...
    float3 param_25;
    float3 _4249 = surface(param_20, param_21, param_22, param_23, param_24, param_25);
    ...
    if (shows > 0.0)
    {
        ...
        bool _4357 = (tHit > 0.0) && (tHit < dist);
        bool _4367;
        if (_4357)
        {
            _4367 = abs(local.x) < (0.4000000059604644775390625 * scale);
        }
        else
        {
            _4367 = _4357;
        ...
        if (_4367)
        {
            _4373 = local.y > (-0.100000001490116119384765625);
        }
        else
        {
            _4373 = _4367;
        ...
        if (_4373)
        {
            _4384 = local.y < (((1.75 * scale) + 0.119999997317790985107421875) + 0.100000001490116119384765625);
        }
        else
        {
            _4384 = _4373;
        ...
        if (_4384)
        {
            ...
            float _4403 = figureDist(param_31, param_32);
            float cover = shows * smoothstep(aa, -aa, _4403);
            if (cover > 0.0)
            {
                float3 param_33 = hit;
...
fragment main0_out main0(constant Globals& _3849 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3849);
    out._fragColor = param;
    return out;
}
"""#
