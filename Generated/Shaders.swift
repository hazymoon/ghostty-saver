...
// shaders-sha256: 20d6a4aa9917d5d7dc0d545de46e5cca1492d2e1f511831a54bb143d699070c2
// prefix-ref: v1.3.1
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3806)
{
    float2 res = float2(_3806.iResolution[0], _3806.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3806.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3806.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_15;
    float3 param_16;
    float _4134 = trace(param_13, param_14, param_15, param_16);
    int id = param_15;
    float3 n = param_16;
    float dist = _4134;
    float3 p = ro + (rd * dist);
    ...
    float3 param_22;
    float3 _4168 = surface(param_17, param_18, param_19, param_20, param_21, param_22);
    ...
    if (shows > 0.0)
    {
        ...
        bool _4276 = (tHit > 0.0) && (tHit < dist);
        bool _4286;
        if (_4276)
        {
            _4286 = abs(local.x) < (0.4000000059604644775390625 * scale);
        }
        else
        {
            _4286 = _4276;
        ...
        if (_4286)
        {
            _4292 = local.y > (-0.100000001490116119384765625);
        }
        else
        {
            _4292 = _4286;
        ...
        if (_4292)
        {
            _4303 = local.y < (((1.75 * scale) + 0.119999997317790985107421875) + 0.100000001490116119384765625);
        }
        else
        {
            _4303 = _4292;
        ...
        if (_4303)
        {
            ...
            float _4321 = figureDist(param_28, param_29);
            float cover = shows * smoothstep(aa, -aa, _4321);
            if (cover > 0.0)
            {
                float3 param_30 = hit;
...
fragment main0_out main0(constant Globals& _3806 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3806);
    out._fragColor = param;
    return out;
}
"""#
