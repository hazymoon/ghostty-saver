...
// shaders-sha256: 55a6f6a4652077f9cbed3b8bc80926c1c902290b8db966b0fed1fa88f025e6ef
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _822[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1228 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1228 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1239 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1239 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1228, _1239);
    float3 _1325;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1325 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1325 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1325;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1131[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1372 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1372 = float2(0.0);
        }
        float2 owner = cell - _1372;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1543 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1543 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1543;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1567 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1567;
    ...
    if (rd.x == 0.0)
    {
        _1611 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1611 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1621 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1621 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1611, _1621);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1675 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1675;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1690 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1690;
...
{
    float2 param = cell;
    bool _1793 = !inBlackout(param);
    bool _1801;
    if (_1793)
    {
        float2 param_1 = cell;
        _1801 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1801 = _1793;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2120 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3198)
{
    float2 res = float2(_3198.iResolution[0], _3198.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3198.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3198.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    ...
    int param_10;
    float3 param_11;
    float _3502 = trace(param_8, param_9, param_10, param_11);
    int id = param_10;
    float3 n = param_11;
    float dist = _3502;
    float3 p = ro + (rd * dist);
    ...
    float param_15 = tNow;
    float3 param_16;
    float3 _3522 = surface(param_12, param_13, param_14, param_15, param_16);
...
fragment main0_out main0(constant Globals& _3198 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3198);
    out._fragColor = param;
    return out;
}
"""#
