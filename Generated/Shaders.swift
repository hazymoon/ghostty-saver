...
// shaders-sha256: 3b4c1d975c9eb6ae03cae68509bfdf47018583862084376115746b6504324a1c
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _440[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}

static inline __attribute__((always_inline))
void smoothPath(thread const float& s, thread float2& pos, thread float2& dir)
{
    pos = float2(0.0);
    dir = float2(0.0);
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _814 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _814 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _826 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _826 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_814, _826);
    float3 _912;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _912 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _912 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _912;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _717[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _959 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _959 = float2(0.0);
        }
        float2 owner = cell - _959;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1130 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1130 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1130;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1154 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1154;
    ...
    if (rd.x == 0.0)
    {
        _1198 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1198 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1208 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1208 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1198, _1208);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1262 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1262;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1277 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1277;
...
{
    float2 param = cell;
    bool _1379 = !inBlackout(param);
    bool _1387;
    if (_1379)
    {
        float2 param_1 = cell;
        _1387 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1387 = _1379;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_1707 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2134)
{
    float2 res = float2(_2134.iResolution[0], _2134.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_2134.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _2134.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    float t = droppedFrames(param) - ((0.5 * stale) / 59.939998626708984375);
    ...
    float u = mod(t, 150.0);
    float param_7 = u;
    float s = (floor(t / 150.0) * 30.0) + walked(param_7);
    float param_8 = u;
    float moving = pace(param_8);
    float param_9 = t * 0.17000000178813934326171875;
    float steps = (s * 6.80000019073486328125) + (0.2199999988079071044921875 * fbm1(param_9));
    ...
    float2 param_11;
    float2 param_12;
    smoothPath(param_10, param_11, param_12);
    float2 body = param_11;
    float2 going = param_12;
    ...
    float param_13 = warped + 0.25;
    float2 param_14;
    float2 param_15;
    smoothPath(param_13, param_14, param_15);
    float2 unused = param_14;
    float2 facing = param_15;
    float neck = precise::atan2(facing.x, facing.y) - heading;
    neck = fast::clamp(precise::atan2(sin(neck), cos(neck)), -0.730000019073486328125, 0.730000019073486328125);
    float turning = fast::clamp(neck / 0.3499999940395355224609375, -1.0, 1.0);
    float param_16 = u;
    float param_17 = 30.0;
    float param_18 = u;
    float param_19 = 106.0;
    float look = (1.60000002384185791015625 * lookBump(param_16, param_17)) - (1.7999999523162841796875 * lookBump(param_18, param_19));
    float yaw = (heading + neck) + look;
    float param_20 = stepNo;
    float param_21 = stepNo + 1.0;
    float amp = mix(stepSize(param_20), stepSize(param_21), su) * gait;
    ...
    float param_22 = sinceStrike;
    float param_23 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + ((0.0040000001899898052215576171875 * gait) * ring(param_22, param_23));
    ...
    float param_24 = sinceStrike;
    float param_25 = 0.07999999821186065673828125;
    float kick = (0.001700000022538006305694580078125 * gait) * ring(param_24, param_25);
    ...
    ruv.y += (1.25 * kick);
    float3 rd = fast::normalize(((forward * 1.25) + (right * ruv.x)) + (up * ruv.y));
    float3 param_26 = ro;
    float3 param_27 = rd;
    int param_28;
    float3 param_29;
    float _2686 = trace(param_26, param_27, param_28, param_29);
    int id = param_28;
    float3 n = param_29;
    float dist = _2686;
    float3 p = ro + (rd * dist);
    ...
    float param_33 = t;
    float3 param_34;
    float3 _2706 = surface(param_30, param_31, param_32, param_33, param_34);
    float3 emission = param_34;
    float3 albedo = _2706;
    float3 param_35 = p;
    float3 param_36 = n;
    float param_37 = t;
    float3 col = (albedo * (lighting(param_35, param_36, param_37) + float3(0.01200000010430812835693359375))) + emission;
    ...
    float2 param_39 = chromaCell + fseed;
    float2 param_40 = (chromaCell + fseed.yx) + float2(7.0);
    float2 blotch = (float2(cheap21(param_39), cheap21(param_40)) * 2.0) - float2(1.0);
    ...
    float2 param_41 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_41);
    float2 param_42 = (lumaCell + fseed) + float2(3.0);
    float2 param_43 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(0.930000007152557373046875, cheap21(param_42)) - step(0.930000007152557373046875, cheap21(param_43));
    Y += (((grain - 0.5) * 0.0599999986588954925537109375) + (snow * (0.017999999225139617919921875 + (band * 0.550000011920928955078125))));
    float2 param_44 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_44);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_45 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_45);
...
fragment main0_out main0(constant Globals& _2134 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _2134);
    out._fragColor = param;
    return out;
}
"""#
