...
// shaders-sha256: 98f364938810f82006860977b682448387c10f738fc1a6dd97dde9e1eae10afe
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _384[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _766 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _766 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _778 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _778 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_766, _778);
    float3 _863;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _863 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _863 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _863;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _661[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _910 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _910 = float2(0.0);
        }
        float2 owner = cell - _910;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1080 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1080 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1080;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1104 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1104;
    ...
    if (rd.x == 0.0)
    {
        _1148 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1148 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1158 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1158 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1148, _1158);
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1212 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1212;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1227 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1227;
...
bool hasTube(thread const float2& cell)
{
    ...
    float2 param_1 = mod(cell, float2(8.0)) + float2(0.10999999940395355224609375, 0.730000019073486328125);
    return hash21(param_1) <= 0.7200000286102294921875;
}
...
float tubeLevel(thread const float2& cell, thread const float& t)
{
    ...
    float2 c = mod(cell, float2(8.0));
    float2 param_1 = c + float2(0.569999992847442626953125, 0.2899999916553497314453125);
    float bad = hash21(param_1);
...
static inline __attribute__((always_inline))
bool litFrom(thread const float2& p, thread const float2& l)
{
    float2 d = l - p;
    float2 cell = floor(p / float2(4.0));
    float2 target = floor(l / float2(4.0));
    float2 _step = sign(d);
    float _1460;
    if (d.x == 0.0)
    {
        _1460 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1460 = d.x;
    ...
    if (d.y == 0.0)
    {
        _1470 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1470 = d.y;
    }
    float2 inv = float2(1.0) / float2(_1460, _1470);
    float2 next = (((cell + fast::max(_step, float2(0.0))) * 4.0) - p) * inv;
    float2 delta = abs(inv * 4.0);
    float _1518;
    int kind;
    float2 _1545;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (axisX)
        {
            ...
            {
                _1545 = cell - float2(1.0, 0.0);
            }
            float2 param = _1545;
            bool param_1 = true;
            kind = wallOn(param, param_1);
            along = fract(q.y / 4.0);
            cell.x += _step.x;
            next.x += delta.x;
        }
        else
        {
            ...
            {
                _1577 = cell - float2(0.0, 1.0);
            }
            float2 param_2 = _1577;
            bool param_3 = false;
            kind = wallOn(param_2, param_3);
            along = fract(q.x / 4.0);
            cell.y += _step.y;
            next.y += delta.y;
        }
...
float3 lighting(thread const float3& p, thread const float3& n, thread const float& t)
{
    ...
    for (int dz = -2; dz <= 2; dz++)
    {
        for (int dx = -2; dx <= 2; dx++)
        {
            ...
            bool _1712 = lambert == 0.0;
            bool _1723;
            if (!_1712)
            {
                float2 param_4 = from;
                float2 param_5 = lp.xz;
                _1723 = !litFrom(param_4, param_5);
            }
            else
            {
                _1723 = _1712;
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2089)
{
    float param = _2089.iTime;
    float t = droppedFrames(param);
    float2 res = float2(_2089.iResolution[0], _2089.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float frameNo = floor(mod(_2089.iTime, 3600.0) * 60.0);
    float param_1 = frameNo + 0.310000002384185791015625;
    float param_2 = frameNo + 0.769999980926513671875;
    float2 fseed = float2(hash11(param_1), hash11(param_2)) * 100.0;
    float param_3 = screenY;
    float param_4 = _2089.iTime;
    ...
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_2089.iTime * 21.0))) * (0.5 + (0.5 * sin(_2089.iTime * 0.699999988079071044921875)));
    ...
    int param_27;
    float3 param_28;
    float _2611 = trace(param_25, param_26, param_27, param_28);
    int id = param_27;
    float3 n = param_28;
    float dist = _2611;
    float3 p = ro + (rd * dist);
    ...
    float param_32 = t;
    float3 param_33;
    float3 _2631 = surface(param_29, param_30, param_31, param_32, param_33);
    ...
    float phase = ((0.14000000059604644775390625 * (hash21(param_37) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    ...
    float2 blotch = (float2(hash21(param_38), hash21(param_39)) * 2.0) - float2(1.0);
    ...
    float2 param_40 = floor(fragCoord / float2(2.5)) + float2(fract(_2089.iTime * 7.0) * 100.0);
    float grain = hash21(param_40);
    float2 param_41 = (lumaCell + fseed) + float2(3.0);
    float2 param_42 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(0.930000007152557373046875, hash21(param_41)) - step(0.930000007152557373046875, hash21(param_42));
    ...
    float bandNoise = hash21(param_43);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_44 = fragCoord + float2(fract(_2089.iTime * 13.0) * 50.0);
    float headSnow = hash21(param_44);
...
fragment main0_out main0(constant Globals& _2089 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _2089);
    out._fragColor = param;
    return out;
}
"""#
