...
// shaders-sha256: ec1e63eb9c48c5ef0e11c8942e06904d2b1aaae4318ddb8568c093ff62593e7f
// prefix-ref: v1.3.1
...
static inline __attribute__((always_inline))
float ditherScale(constant Globals& _95)
{
    return fast::max(1.0, floor(_95.iResolution[1u] / 720.0));
}
...
static inline __attribute__((always_inline))
float3 dither555(thread const float3& c, thread const float2& fragCoord, constant Globals& _95)
{
    float2 param = floor(fragCoord / float2(ditherScale(_95)));
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _95)
{
    float2 uv = fragCoord / float2(_95.iResolution[0], _95.iResolution[1]);
    float pulse = 0.5 + (0.5 * sin(_95.iTime * 2.0));
    float3 param = float3(uv.x, uv.y, pulse);
    float2 param_1 = fragCoord;
    fragColor = float4(dither555(param, param_1, _95), 1.0);
}

fragment main0_out main0(constant Globals& _95 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _95);
    out._fragColor = param;
    return out;
}
"""#
