...
// shaders-sha256: 188fe7089c4e248cba2efe7ad2064caf2b06851ffb912dbe339e83f89443e1e2
// prefix-ref: v1.3.1
...
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2084)
{
    ...
    uv.y = -uv.y;
    float frameNo = floor(mod(_2084.iTime, 3600.0) * 60.0);
    float param_1 = frameNo + 0.310000002384185791015625;
    float param_2 = frameNo + 0.769999980926513671875;
    float2 fseed = floor(float2(hash11(param_1), hash11(param_2)) * 100.0);
    float param_3 = screenY;
    float param_4 = _2084.iTime;
    ...
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_2084.iTime * 21.0))) * (0.5 + (0.5 * sin(_2084.iTime * 0.699999988079071044921875)));
    ...
    int param_27;
    float3 param_28;
    float _2608 = trace(param_25, param_26, param_27, param_28);
    int id = param_27;
    float3 n = param_28;
    float dist = _2608;
    float3 p = ro + (rd * dist);
    ...
    float param_32 = t;
    float3 param_33;
    float3 _2628 = surface(param_29, param_30, param_31, param_32, param_33);
    ...
    float sub = ((6.283185482025146484375 * inFrame.x) * 0.125) + (3.1415927410125732421875 * (line + frameNo));
    ...
    float2 param_40 = floor(fragCoord / float2(2.5)) + float2(floor(fract(_2084.iTime * 7.0) * 100.0));
    ...
    float2 param_44 = floor(fragCoord) + float2(floor(fract(_2084.iTime * 13.0) * 50.0));
