...
// shaders-sha256: 48f86b5c5a5081ef0985f34cf6cd6f1e1f5078f69e0166c8ee5c74b71445e6b3
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _370[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _752 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _752 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _764 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _764 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_752, _764);
    float3 _849;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _849 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _849 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _849;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _647[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _873 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _873;
    ...
    if (rd.x == 0.0)
    {
        _917 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _917 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _927 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _927 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_917, _927);
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _972 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _972;
        ...
        if (axisX)
        {
            ...
            {
                _1023 = cell - float2(1.0, 0.0);
            }
            float2 wallCell = _1023;
        ...
        else
        {
            ...
            {
                _1046 = cell - float2(0.0, 1.0);
            }
            float2 wallCell_1 = _1046;
        ...
        if (wallCovers(param_8, param_9))
        {
            ...
            if (axisX)
            {
                _1071 = float3(-_step.x, 0.0, 0.0);
            }
            else
            {
                _1071 = float3(0.0, 0.0, -_step.y);
            }
            normal = _1071;
            return exit;
        }
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _1684)
{
    float param = _1684.iTime;
    float t = droppedFrames(param);
    float2 res = float2(_1684.iResolution[0], _1684.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float frameNo = floor(mod(_1684.iTime, 3600.0) * 60.0);
    ...
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_1684.iTime * 21.0))) * (0.5 + (0.5 * sin(_1684.iTime * 0.699999988079071044921875)));
    ...
    float warped = s - (((moving * 0.64999997615814208984375) / 21.3628292083740234375) * cos(6.283185482025146484375 * strideP));
    ...
    int param_27;
    float3 param_28;
    float _2208 = trace(param_25, param_26, param_27, param_28);
    int id = param_27;
    float3 n = param_28;
    float dist = _2208;
    float3 p = ro + (rd * dist);
    ...
    float param_32 = t;
    float3 param_33;
    float3 _2228 = surface(param_29, param_30, param_31, param_32, param_33);
    ...
    float2 param_40 = floor(fragCoord / float2(2.5)) + float2(fract(_1684.iTime * 7.0) * 100.0);
    ...
    float2 param_44 = fragCoord + float2(fract(_1684.iTime * 13.0) * 50.0);
...
fragment main0_out main0(constant Globals& _1684 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _1684);
    out._fragColor = param;
    return out;
}
"""#
