...
// shaders-sha256: a7d5a8d30f8c9f3233a87e3e88cbf616d26e2a7b4c2c8bbfa7abed855f035af9
// prefix-ref: v1.3.1
...
{
    float speed = 0.226415097713470458984375;
    float param = u;
    float param_1 = 30.0;
    float param_2 = u;
    float param_3 = 113.0;
...
float headingAt(thread const float& s)
{
    ...
    for (int k = -2; k <= 2; k++)
    {
        float x = (s + 0.20000000298023223876953125) + ((float(k) * 1.5) * 0.20000000298023223876953125);
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _563 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _563 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _575 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _575 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_563, _575);
    float3 _660;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _660 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _660 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _660;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _458[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _684 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _684;
    ...
    if (rd.x == 0.0)
    {
        _728 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _728 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _738 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _738 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_728, _738);
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _783 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _783;
        ...
        if (axisX)
        {
            ...
            {
                _835 = cell - float2(1.0, 0.0);
            }
            float2 wallCell = _835;
        ...
        else
        {
            ...
            {
                _858 = cell - float2(0.0, 1.0);
            }
            float2 wallCell_1 = _858;
        ...
        if (wallCovers(param_8, param_9))
        {
            ...
            if (axisX)
            {
                _883 = float3(-_step.x, 0.0, 0.0);
            }
            else
            {
                _883 = float3(0.0, 0.0, -_step.y);
            }
            normal = _883;
            return exit;
        }
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _1498)
{
    float param = _1498.iTime;
    float t = droppedFrames(param);
    float2 res = float2(_1498.iResolution[0], _1498.iResolution[1]);
    float mask = 1.0;
    ...
    float param_2 = _1498.iTime;
    float band = trackingBand(param_1, param_2);
    float param_3 = floor(screenY * 90.0) + floor(_1498.iTime * 30.0);
    ...
    float param_4 = floor(_1498.iTime * 60.0);
    float headShift = (headSwitch * 0.02999999932944774627685546875) * (hash11(param_4) - 0.5);
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_1498.iTime * 21.0))) * (0.5 + (0.5 * sin(_1498.iTime * 0.699999988079071044921875)));
    ...
    float param_6 = s - 0.4000000059604644775390625;
    float param_7 = s + 0.4000000059604644775390625;
    float2 pos2 = (onPath(param_6) + onPath(param_7)) * 0.5;
    float param_8 = s;
    float heading = headingAt(param_8);
    float param_9 = u;
    float param_10 = 30.0;
    float param_11 = u;
    float param_12 = 113.0;
    float look = (1.60000002384185791015625 * lookBump(param_9, param_10)) - (1.7999999523162841796875 * lookBump(param_11, param_12));
    float yaw = heading + look;
    float param_13 = u;
    float moving = pace(param_13);
    float stride = (s * 6.80000019073486328125) * 3.1415927410125732421875;
    float bob = (0.02500000037252902984619140625 * moving) * sin(stride * 2.0);
    float sway = (0.0199999995529651641845703125 * moving) * sin(stride);
    float roll = (0.007000000216066837310791015625 * moving) * sin(stride);
    float2 side = float2(cos(heading), -sin(heading));
    float3 head = float3((pos2.x * 4.0) + 2.0, 1.60000002384185791015625, (pos2.y * 4.0) + 2.0);
    float3 ro = head + float3(side.x * sway, bob, side.y * sway);
    float3 target = head + (float3(sin(yaw), 0.0, cos(yaw)) * 6.0);
    float3 forward = fast::normalize(target - ro);
    ...
    int param_16;
    float3 param_17;
    float _1830 = trace(param_14, param_15, param_16, param_17);
    int id = param_16;
    float3 n = param_17;
    float dist = _1830;
    float3 p = ro + (rd * dist);
    ...
    float param_21 = t;
    float3 param_22;
    float3 _1850 = surface(param_18, param_19, param_20, param_21, param_22);
    float3 emission = param_22;
    float3 albedo = _1850;
    float3 param_23 = p;
    float3 param_24 = n;
    float param_25 = t;
    float3 col = (albedo * (lighting(param_23, param_24, param_25) + float3(0.01200000010430812835693359375))) + emission;
    ...
    float2 param_26 = floor(fragCoord / float2(1.5)) + float2(fract(_1498.iTime * 7.0) * 100.0);
    float grain = hash21(param_26);
    col += float3((grain - 0.5) * 0.0900000035762786865234375);
    float2 param_27 = float2(floor(screenY * 240.0), floor(_1498.iTime * 30.0));
    float bandNoise = hash21(param_27);
    ...
    float2 param_28 = fragCoord + float2(fract(_1498.iTime * 13.0) * 50.0);
    float snow = hash21(param_28);
...
fragment main0_out main0(constant Globals& _1498 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _1498);
    out._fragColor = param;
    return out;
}
"""#
