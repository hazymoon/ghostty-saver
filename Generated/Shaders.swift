...
// shaders-sha256: c3064682ed1522697e060c78e565356693ad117d91f948c7ff44c8bdd0ef0cca
// prefix-ref: v1.3.1
...
static inline __attribute__((always_inline))
float headingAt(thread const float& s)
{
    float2 dir = float2(0.0);
    for (int k = -2; k <= 2; k++)
    {
        float x = (s + 0.25) + ((float(k) * 1.5) * 0.20000000298023223876953125);
        float param = x + 0.1500000059604644775390625;
        float param_1 = x - 0.1500000059604644775390625;
...
float turnRate(thread const float& s)
{
    ...
    float param_1 = s - 0.0500000007450580596923828125;
    float d = headingAt(param) - headingAt(param_1);
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _689 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _689 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _701 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _701 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_689, _701);
    float3 _786;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _786 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _786 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _786;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _584[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _810 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _810;
    ...
    if (rd.x == 0.0)
    {
        _854 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _854 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _864 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _864 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_854, _864);
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _909 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _909;
        ...
        if (axisX)
        {
            ...
            {
                _960 = cell - float2(1.0, 0.0);
            }
            float2 wallCell = _960;
        ...
        else
        {
            ...
            {
                _983 = cell - float2(0.0, 1.0);
            }
            float2 wallCell_1 = _983;
        ...
        if (wallCovers(param_8, param_9))
        {
            ...
            if (axisX)
            {
                _1008 = float3(-_step.x, 0.0, 0.0);
            }
            else
            {
                _1008 = float3(0.0, 0.0, -_step.y);
            }
            normal = _1008;
            return exit;
        }
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _1619)
{
    float param = _1619.iTime;
    float t = droppedFrames(param);
    float2 res = float2(_1619.iResolution[0], _1619.iResolution[1]);
    float mask = 1.0;
    ...
    float param_2 = _1619.iTime;
    float band = trackingBand(param_1, param_2);
    float param_3 = floor(screenY * 90.0) + floor(_1619.iTime * 30.0);
    ...
    float param_4 = floor(_1619.iTime * 60.0);
    float headShift = (headSwitch * 0.02999999932944774627685546875) * (hash11(param_4) - 0.5);
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_1619.iTime * 21.0))) * (0.5 + (0.5 * sin(_1619.iTime * 0.699999988079071044921875)));
    ...
    float param_10 = s - 0.4000000059604644775390625;
    float param_11 = s + 0.4000000059604644775390625;
    float2 pos2 = (onPath(param_10) + onPath(param_11)) * 0.5;
    float warped = s - (((moving * 0.64999997615814208984375) / 21.3628292083740234375) * cos(6.283185482025146484375 * strideP));
    float param_12 = warped;
    float heading = headingAt(param_12);
    float param_13 = warped;
    float turning = fast::clamp(turnRate(param_13) / 1.5, -1.0, 1.0);
    float param_14 = u;
    float param_15 = 30.0;
    float param_16 = u;
    float param_17 = 113.0;
    float look = (1.60000002384185791015625 * lookBump(param_14, param_15)) - (1.7999999523162841796875 * lookBump(param_16, param_17));
    ...
    float param_18 = sinceStrike;
    float param_19 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + ((0.0040000001899898052215576171875 * moving) * ring(param_18, param_19));
    ...
    float param_21 = 0.07999999821186065673828125;
    float kick = (0.001700000022538006305694580078125 * moving) * ring(param_20, param_21);
    float3 side = float3(cos(heading), 0.0, -sin(heading));
    float3 ahead = float3(sin(heading), 0.0, cos(heading));
    float3 head = float3((pos2.x * 4.0) + 2.0, 1.60000002384185791015625, (pos2.y * 4.0) + 2.0);
    ...
    int param_24;
    float3 param_25;
    float _2089 = trace(param_22, param_23, param_24, param_25);
    int id = param_24;
    float3 n = param_25;
    float dist = _2089;
    float3 p = ro + (rd * dist);
    ...
    float param_29 = t;
    float3 param_30;
    float3 _2109 = surface(param_26, param_27, param_28, param_29, param_30);
    float3 emission = param_30;
    float3 albedo = _2109;
    float3 param_31 = p;
    float3 param_32 = n;
    float param_33 = t;
    float3 col = (albedo * (lighting(param_31, param_32, param_33) + float3(0.01200000010430812835693359375))) + emission;
    ...
    float2 param_34 = floor(fragCoord / float2(1.5)) + float2(fract(_1619.iTime * 7.0) * 100.0);
    float grain = hash21(param_34);
    col += float3((grain - 0.5) * 0.0900000035762786865234375);
    float2 param_35 = float2(floor(screenY * 240.0), floor(_1619.iTime * 30.0));
    float bandNoise = hash21(param_35);
    ...
    float2 param_36 = fragCoord + float2(fract(_1619.iTime * 13.0) * 50.0);
    float snow = hash21(param_36);
...
fragment main0_out main0(constant Globals& _1619 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _1619);
    out._fragColor = param;
    return out;
}
"""#
