...
// shaders-sha256: 9d4bb79619cef8b0842fd32a497d1322244a5eefaf0fcc06bf65710779d97888
// prefix-ref: v1.3.1
...
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _1684)
{
    ...
    float param_2 = _1684.iTime;
    float band = trackingBand(param_1, param_2);
    float param_3 = floor(screenY * 90.0) + floor(_1684.iTime * 30.0);
    ...
    float param_4 = floor(_1684.iTime * 60.0);
    ...
    float u = mod(t, 150.0);
    float param_5 = u;
    float s = (floor(t / 150.0) * 30.0) + walked(param_5);
    float param_6 = u;
    ...
    float2 param_9;
    float2 param_10;
    smoothPath(param_8, param_9, param_10);
    float2 body = param_9;
    float2 going = param_10;
    ...
    float param_11 = warped + 0.25;
    float2 param_12;
    float2 param_13;
    smoothPath(param_11, param_12, param_13);
    float2 unused = param_12;
    float2 facing = param_13;
    ...
    float param_18 = stepNo;
    float param_19 = stepNo + 1.0;
    float amp = mix(stepSize(param_18), stepSize(param_19), su) * gait;
    ...
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + ((0.0040000001899898052215576171875 * gait) * ring(param_20, param_21));
    ...
    float kick = (0.001700000022538006305694580078125 * gait) * ring(param_22, param_23);
    ...
    int param_26;
    float3 param_27;
    float _2192 = trace(param_24, param_25, param_26, param_27);
    int id = param_26;
    float3 n = param_27;
    float dist = _2192;
    float3 p = ro + (rd * dist);
    ...
    float param_31 = t;
    float3 param_32;
    float3 _2212 = surface(param_28, param_29, param_30, param_31, param_32);
    ...
    float2 param_36 = floor(fragCoord / float2(1.5)) + float2(fract(_1684.iTime * 7.0) * 100.0);
    float grain = hash21(param_36);
    col += float3((grain - 0.5) * 0.0900000035762786865234375);
    float2 param_37 = float2(floor(screenY * 240.0), floor(_1684.iTime * 30.0));
    ...
    float2 param_38 = fragCoord + float2(fract(_1684.iTime * 13.0) * 50.0);
