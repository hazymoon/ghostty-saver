...
// shaders-sha256: 5f9edd15edbf2e22f85b7e638ef81adc36b3917878135f3231e5eded3b7dfa69
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _437[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _811 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _811 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _823 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _823 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_811, _823);
    float3 _909;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _909 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _909 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _909;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _714[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _956 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _956 = float2(0.0);
        }
        float2 owner = cell - _956;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1127 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1127 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1127;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1151 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1151;
    ...
    if (rd.x == 0.0)
    {
        _1195 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1195 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1205 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1205 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1195, _1205);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1259 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1259;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1274 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1274;
...
{
    float2 param = cell;
    bool _1376 = !inBlackout(param);
    bool _1384;
    if (_1376)
    {
        float2 param_1 = cell;
        _1384 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1384 = _1376;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_1704 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2084)
{
    float2 res = float2(_2084.iResolution[0], _2084.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_2084.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _2084.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    ...
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (now * 21.0))) * (0.5 + (0.5 * sin(now * 0.699999988079071044921875)));
    uv.x += ((bandShift + headShift) + wobble);
    float r2 = dot(uv, uv);
    uv *= (1.0 + (0.0500000007450580596923828125 * r2));
    float u = mod(t, 150.0);
    float param_6 = u;
    float s = (floor(t / 150.0) * 30.0) + walked(param_6);
    float param_7 = u;
    float moving = pace(param_7);
    float param_8 = t * 0.17000000178813934326171875;
    float steps = (s * 6.80000019073486328125) + (0.2199999988079071044921875 * fbm1(param_8));
    ...
    float2 param_10;
    float2 param_11;
    smoothPath(param_9, param_10, param_11);
    float2 body = param_10;
    float2 going = param_11;
    ...
    float param_12 = warped + 0.25;
    float2 param_13;
    float2 param_14;
    smoothPath(param_12, param_13, param_14);
    float2 unused = param_13;
    float2 facing = param_14;
    ...
    float look = (1.60000002384185791015625 * lookBump(param_15, param_16)) - (1.7999999523162841796875 * lookBump(param_17, param_18));
    ...
    float kick = (0.001700000022538006305694580078125 * gait) * ring(param_23, param_24);
    ...
    int param_27;
    float3 param_28;
    float _2638 = trace(param_25, param_26, param_27, param_28);
    int id = param_27;
    float3 n = param_28;
    float dist = _2638;
    float3 p = ro + (rd * dist);
    ...
    float param_32 = t;
    float3 param_33;
    float3 _2658 = surface(param_29, param_30, param_31, param_32, param_33);
    float3 emission = param_33;
    float3 albedo = _2658;
    float3 param_34 = p;
    float3 param_35 = n;
    float param_36 = t;
    float3 col = (albedo * (lighting(param_34, param_35, param_36) + float3(0.01200000010430812835693359375))) + emission;
    ...
    C += (dyiq.yz * 6.0);
    float2 param_37 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_37) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_38 = chromaCell + fseed;
    float2 param_39 = (chromaCell + fseed.yx) + float2(7.0);
    float2 blotch = (float2(cheap21(param_38), cheap21(param_39)) * 2.0) - float2(1.0);
    ...
    float snow = step(0.930000007152557373046875, cheap21(param_41)) - step(0.930000007152557373046875, cheap21(param_42));
...
fragment main0_out main0(constant Globals& _2084 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _2084);
    out._fragColor = param;
    return out;
}
"""#
