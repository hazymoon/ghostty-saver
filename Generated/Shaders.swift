...
// shaders-sha256: 3409e02e1f08fcdedbd39b2428a12a719abf49df9f73e25caea7de49e5f791ca
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 4> _477 = spvUnsafeArray<float4, 4>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(22.1000003814697265625, 9.8999996185302734375, 2.0, 0.449999988079071044921875), float4(38.0, 18.799999237060546875, 2.0, 0.449999988079071044921875) });
...
Pose cameraPose(thread const float& t)
{
    ...
    float param_19 = u;
    float2 param_20 = (body + float2(0.5)) * 4.0;
    float2 param_21 = (going * 4.0) * sRate;
    float param_22 = heading + neck;
    float param_23 = headingRate + neckRate;
    float param_24;
    float param_25;
    gazeOffset(param_19, param_20, param_21, param_22, param_23, param_24, param_25);
    float gazeOff = param_24;
    float gazeRate = param_25;
    look += gazeOff;
    lookRate += gazeRate;
    float yaw = (heading + neck) + look;
    float yawRate = (headingRate + neckRate) + lookRate;
    float param_26 = u;
    float fear = fright(param_26);
    float braced = mix(1.0, 2.2000000476837158203125, fear);
    float vor = mix(1.0, 0.300000011920928955078125, fear);
    float2 param_27 = body;
    float quietly = care(param_27);
    ...
    float param_28 = stepNo;
    float param_29 = stepNo + 1.0;
    float amp = ((mix(stepSize(param_28), stepSize(param_29), su) * gait) * braced) * mix(1.0, 0.7200000286102294921875, quietly);
    ...
    float param_30 = sinceStrike;
    float param_31 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((arch2 * cos(12.56637096405029296875 * sw)) + (arch3 * cos(18.84955596923828125 * sw)))) + (strike * ring(param_30, param_31));
    float param_32 = sinceStrike;
    float param_33 = 0.070000000298023223876953125;
    float bobRate = ((((0.02500000037252902984619140625 * amp) * arch) * swRate) * (((6.283185482025146484375 * sin(6.283185482025146484375 * sw)) + ((arch2 * 12.56637096405029296875) * sin(12.56637096405029296875 * sw))) + ((arch3 * 18.84955596923828125) * sin(18.84955596923828125 * sw)))) + ((strike * ringRate(param_32, param_33)) * moving);
    ...
    float param_34 = sinceStrike;
    float param_35 = 0.07999999821186065673828125;
    float kick = (((0.001700000022538006305694580078125 * gait) * braced) * quiet) * ring(param_34, param_35);
    float param_36 = sinceStrike;
    float param_37 = 0.07999999821186065673828125;
    float kickRate = ((((0.001700000022538006305694580078125 * gait) * braced) * quiet) * ringRate(param_36, param_37)) * moving;
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3224)
{
    float2 res = float2(_3224.iResolution[0], _3224.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3224.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3224.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    ...
    int param_10;
    float3 param_11;
    float _3528 = trace(param_8, param_9, param_10, param_11);
    int id = param_10;
    float3 n = param_11;
    float dist = _3528;
    float3 p = ro + (rd * dist);
    ...
    float param_15 = tNow;
    float3 param_16;
    float3 _3548 = surface(param_12, param_13, param_14, param_15, param_16);
...
fragment main0_out main0(constant Globals& _3224 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3224);
    out._fragColor = param;
    return out;
}
"""#
