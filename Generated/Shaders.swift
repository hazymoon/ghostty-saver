...
// shaders-sha256: b68b5c843d1b33f0091cdb80ff15de5cbdb6318d134e71a4d5296b217bf6af55
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 5> _480 = spvUnsafeArray<float4, 5>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(21.729999542236328125, 10.270000457763671875, 2.0, 0.449999988079071044921875), float4(38.0, 17.8999996185302734375, 2.0, 0.449999988079071044921875), float4(130.3000030517578125, 3.0, 0.60000002384185791015625, 1.0) });
constant spvUnsafeArray<float4, 1> _656 = spvUnsafeArray<float4, 1>({ float4(6.0, 5.0, 40.0, 6.5) });
constant spvUnsafeArray<float2, 31> _825 = spvUnsafeArray<float2, 31>({ float2(1.0, 0.0), float2(1.0), float2(1.0, 2.0), float2(2.0), float2(3.0, 2.0), float2(4.0, 2.0), float2(4.0, 3.0), float2(4.0), float2(4.0, 5.0), float2(4.0, 6.0), float2(4.0, 7.0), float2(4.0, 8.0), float2(4.0, 9.0), float2(4.0, 10.0), float2(3.0, 10.0), float2(2.0, 10.0), float2(1.0, 10.0), float2(0.0, 10.0), float2(-1.0, 10.0), float2(-2.0, 10.0), float2(-3.0, 10.0), float2(-3.0, 9.0), float2(-3.0, 8.0), float2(-3.0, 7.0), float2(-3.0, 6.0), float2(-2.0, 6.0), float2(-1.0, 6.0), float2(0.0, 6.0), float2(1.0, 6.0), float2(1.0, 7.0), float2(1.0, 8.0) });
constant spvUnsafeArray<int, 64> _1134 = spvUnsafeArray<int, 64>({ 0, 10, 0, 0, 10, 10, 0, 0, 0, 10, 0, 0, 10, 10, 0, 0, 5, 13, 5, 5, 14, 9, 5, 5, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 5, 6, 0, 0, 10, 3, 5, 5, 0, 10, 0, 0, 10, 10, 0, 0 });
...
float droppedFrames(thread const float& t)
{
    ...
    float hold = step(h, 0.20000000298023223876953125) * 0.25;
...
float trackingBand(thread const float& y, thread const float& t)
{
    ...
    float h = hash11(param);
    float rolling = step(h, 0.20000000298023223876953125) * step(within, 2.0);
...
static inline __attribute__((always_inline))
float timeBaseError(thread const float& t)
{
    ...
    float since = t - start;
    float on = (step(h, 0.25) * step(0.0, since)) * step(since, 4.0);
...
float2 waypoint(thread const float& k)
{
    ...
    return _825[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
{
    float2 param = cell;
    bool _1796 = !inBlackout(param);
    bool _1804;
    if (_1796)
    {
        float2 param_1 = cell;
        _1804 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1804 = _1796;
...
static inline __attribute__((always_inline))
float care(thread const float2& body)
{
    float lit = 0.0;
    float total = 0.0;
    float2 base = floor(body) - float2(1.0);
    for (int k = 0; k < 16; k++)
    {
        float2 c = base + float2(float(k & 3), float(k >> 2));
        float2 w2 = fast::max(float2(1.0) - (abs(c - body) * 0.5), float2(0.0));
        float w = w2.x * w2.y;
        float2 param = c;
        lit += (w * float(hasTube(param)));
        total += w;
    }
    return smoothstep(0.37999999523162841796875, 0.62000000476837158203125, 1.0 - (lit / total));
}
...
Pose cameraPose(thread const float& t)
{
    ...
    float g0 = 82.26999664306640625;
    float g1 = 84.06999969482421875;
    float g2 = 85.87000274658203125;
    float g3 = 88.26999664306640625;
    float param_19 = u;
    float param_20 = g0;
    float param_21 = g1;
    float param_22 = g2;
    ...
    float2 param_37 = body;
    float quietly = care(param_37);
    ...
    float param_38 = stepNo;
    float param_39 = stepNo + 1.0;
    float amp = ((mix(stepSize(param_38), stepSize(param_39), su) * gait) * braced) * mix(1.0, 0.7200000286102294921875, quietly);
    ...
    float param_40 = sinceStrike;
    float param_41 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((arch2 * cos(12.56637096405029296875 * sw)) + (arch3 * cos(18.84955596923828125 * sw)))) + (strike * ring(param_40, param_41));
    float param_42 = sinceStrike;
    float param_43 = 0.070000000298023223876953125;
    float bobRate = ((((0.02500000037252902984619140625 * amp) * arch) * swRate) * (((6.283185482025146484375 * sin(6.283185482025146484375 * sw)) + ((arch2 * 12.56637096405029296875) * sin(12.56637096405029296875 * sw))) + ((arch3 * 18.84955596923828125) * sin(18.84955596923828125 * sw)))) + ((strike * ringRate(param_42, param_43)) * moving);
    ...
    float param_44 = sinceStrike;
    float param_45 = 0.07999999821186065673828125;
    float kick = (((0.001700000022538006305694580078125 * gait) * braced) * quiet) * ring(param_44, param_45);
    float param_46 = sinceStrike;
    float param_47 = 0.07999999821186065673828125;
    float kickRate = ((((0.001700000022538006305694580078125 * gait) * braced) * quiet) * ringRate(param_46, param_47)) * moving;
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1231 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1231 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1242 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1242 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1231, _1242);
    float3 _1328;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1328 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1328 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1328;
        }
    }
...
{
    float2 c = mod(cell, float2(8.0));
    int mask = _1134[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1375 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1375 = float2(0.0);
        }
        float2 owner = cell - _1375;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1546 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1546 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1546;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1570 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1570;
    ...
    if (rd.x == 0.0)
    {
        _1614 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1614 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1624 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1624 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1614, _1624);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1678 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1678;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1693 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1693;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2123 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3264)
{
    float2 res = float2(_3264.iResolution[0], _3264.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3264.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3264.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    float tNow = droppedFrames(param);
    float param_1 = fieldNo + 0.310000002384185791015625;
    float param_2 = fieldNo + 0.769999980926513671875;
    float2 fseed = floor(float2(hash11(param_1), hash11(param_2)) * 100.0);
    float param_3 = screenY;
    float param_4 = now;
    float band = trackingBand(param_3, param_4);
    float param_5 = floor(screenY * 90.0) + fseed.x;
    float bandShift = band * (0.0500000007450580596923828125 + (0.039999999105930328369140625 * hash11(param_5)));
    ...
    float param_6 = now;
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (now * 21.0))) * timeBaseError(param_6);
    uv.x += ((bandShift + headShift) + wobble);
    float r2 = dot(uv, uv);
    uv *= (1.0 + (0.0500000007450580596923828125 * r2));
    float param_7 = tNow;
    Pose cam = cameraPose(param_7);
    ...
    int param_10;
    float3 param_11;
    float _3568 = trace(param_8, param_9, param_10, param_11);
    int id = param_10;
    float3 n = param_11;
    float dist = _3568;
    float3 p = ro + (rd * dist);
    ...
    float param_15 = tNow;
    float3 param_16;
    float3 _3588 = surface(param_12, param_13, param_14, param_15, param_16);
    float3 emission = param_16;
    float3 albedo = _3588;
    float3 param_17 = p;
    float3 param_18 = n;
    float param_19 = tNow;
    float3 col = (albedo * (lighting(param_17, param_18, param_19) + float3(0.01200000010430812835693359375))) + emission;
    ...
    C += (dyiq.yz * 6.0);
    float2 param_20 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_20) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_21 = chromaCell + fseed;
    float2 param_22 = (chromaCell + fseed.yx) + float2(7.0);
    float2 blotch = (float2(cheap21(param_21), cheap21(param_22)) * 2.0) - float2(1.0);
    C *= (1.0 - (band * 0.85000002384185791015625));
    C += (blotch * (0.039999999105930328369140625 + (bandEdge * 0.3499999940395355224609375)));
    ...
    float2 param_23 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_23);
    float2 param_24 = (lumaCell + fseed) + float2(3.0);
    float2 param_25 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(0.930000007152557373046875, cheap21(param_24)) - step(0.930000007152557373046875, cheap21(param_25));
    Y += (((grain - 0.5) * 0.0599999986588954925537109375) + (snow * (0.017999999225139617919921875 + (band * 0.550000011920928955078125))));
    float2 param_26 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_26);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_27 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_27);
...
fragment main0_out main0(constant Globals& _3264 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3264);
    out._fragColor = param;
    return out;
}
"""#
