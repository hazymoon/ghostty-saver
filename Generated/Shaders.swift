...
// shaders-sha256: deab609cdda4b51ca335af090e8e9bcdbe88095930e2dbe0c1c9a3af210ebf03
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 7> _560 = spvUnsafeArray<float4, 7>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(23.229999542236328125, 8.770000457763671875, 2.0, 0.449999988079071044921875), float4(38.0, 20.8899993896484375, 2.0, 0.449999988079071044921875), float4(132.25, 3.0, 0.60000002384185791015625, 1.0), float4(56.8899993896484375, 22.0, 2.5, -0.300000011920928955078125), float4(135.25, 12.0, 1.5, -0.300000011920928955078125) });
constant spvUnsafeArray<float4, 1> _764 = spvUnsafeArray<float4, 1>({ float4(6.0, 5.0, 40.0, 6.5) });
constant spvUnsafeArray<float2, 31> _933 = spvUnsafeArray<float2, 31>({ float2(1.0, 0.0), float2(1.0), float2(1.0, 2.0), float2(2.0), float2(3.0, 2.0), float2(4.0, 2.0), float2(4.0, 3.0), float2(4.0), float2(4.0, 5.0), float2(4.0, 6.0), float2(4.0, 7.0), float2(4.0, 8.0), float2(4.0, 9.0), float2(4.0, 10.0), float2(3.0, 10.0), float2(2.0, 10.0), float2(1.0, 10.0), float2(0.0, 10.0), float2(-1.0, 10.0), float2(-2.0, 10.0), float2(-3.0, 10.0), float2(-3.0, 9.0), float2(-3.0, 8.0), float2(-3.0, 7.0), float2(-3.0, 6.0), float2(-2.0, 6.0), float2(-1.0, 6.0), float2(0.0, 6.0), float2(1.0, 6.0), float2(1.0, 7.0), float2(1.0, 8.0) });
constant spvUnsafeArray<int, 64> _1242 = spvUnsafeArray<int, 64>({ 0, 10, 0, 0, 10, 10, 0, 0, 0, 10, 0, 0, 10, 10, 0, 0, 5, 13, 5, 5, 14, 9, 5, 5, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 5, 6, 0, 0, 10, 3, 5, 5, 0, 10, 0, 0, 10, 10, 0, 0 });
...
{
    float param = u;
    float param_1 = 131.4499969482421875;
    float param_2 = 132.350006103515625;
    float param_3 = 138.5500030517578125;
    float param_4 = 139.149993896484375;
...
float2 waypoint(thread const float& k)
{
    ...
    return _933[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
void smoothPath(thread const float& s, thread float2& pos, thread float2& dir, thread float2& turn)
{
    ...
    for (float k = k0; k <= k1; k += 1.0)
    {
        ...
        for (int part = 0; part < 2; part++)
        {
            ...
            {
                _1013 = fast::max(a, 0.0);
            }
            float ta = _1013;
            if (part == 0)
            {
                _1024 = fast::min(b, 0.0);
            }
            else
            {
                _1024 = b;
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1339 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1339 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1350 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1350 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1339, _1350);
    float3 _1436;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1436 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1436 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1436;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1242[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1483 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1483 = float2(0.0);
        }
        float2 owner = cell - _1483;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1654 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1654 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1654;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1678 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1678;
    ...
    if (rd.x == 0.0)
    {
        _1722 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1722 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1732 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1732 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1722, _1732);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1786 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1786;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1801 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1801;
...
{
    float2 param = cell;
    bool _1904 = !inBlackout(param);
    bool _1912;
    if (_1904)
    {
        float2 param_1 = cell;
        _1912 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1912 = _1904;
...
{
    if ((u < 131.0500030517578125) || (u > 142.6999969482421875))
    {
        return 1.0;
    }
    float back = 138.5500030517578125 + (3.0 * fract(h * 7.30000019073486328125));
    ...
    return mix(1.0, lit, smoothstep(131.0500030517578125, 131.1100006103515625, u));
}
...
float tubeLevel(thread const float2& cell, thread const float& t)
{
    ...
    float param_3 = h;
    float power = tubeMains(param_2, param_3);
    if (power == 0.0)
    {
        return 0.0;
    }
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2327 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3491)
{
    float2 res = float2(_3491.iResolution[0], _3491.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3491.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3491.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    float gain = 1.0 + (2.2000000476837158203125 * agcAt(param_5));
    ...
    int param_15;
    float3 param_16;
    float _3814 = trace(param_13, param_14, param_15, param_16);
    int id = param_15;
    float3 n = param_16;
    float dist = _3814;
    float3 p = ro + (rd * dist);
    ...
    float3 param_22;
    float3 _3848 = surface(param_17, param_18, param_19, param_20, param_21, param_22);
    ...
    C += (dyiq.yz * 6.0);
    float2 param_26 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_26) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_27 = chromaAt + fseed;
    float2 blotch = (_noise2(param_27) * 2.0) - float2(1.0);
    C *= (1.0 - (band * 0.85000002384185791015625));
    C += (blotch * (((0.039999999105930328369140625 * (1.0 + (storm * 1.0))) * gain) + (bandEdge * 0.3499999940395355224609375)));
    ...
    float2 param_28 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_28);
    float snowGate = 0.930000007152557373046875 - (storm * 0.119999997317790985107421875);
    float2 param_29 = (lumaCell + fseed) + float2(3.0);
    float2 param_30 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(snowGate, cheap21(param_29)) - step(snowGate, cheap21(param_30));
    Y += (((((grain - 0.5) * 0.0599999986588954925537109375) * (1.0 + (storm * 1.0))) * gain) + (snow * (((0.017999999225139617919921875 * (1.0 + (storm * 1.0))) * gain) + (band * 0.550000011920928955078125))));
    float2 param_31 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_31);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_32 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_32);
...
fragment main0_out main0(constant Globals& _3491 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3491);
    out._fragColor = param;
    return out;
}
"""#
