...
// shaders-sha256: f0c05e4c45c3969cea3d555c37bee7848188c68934ea228652f5a9b0ae6871ea
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 2> _798 = spvUnsafeArray<float4, 2>({ float4(6.0, 5.0, 40.0, 6.5), float4(3.0, 6.0, 124.0, 8.0) });
...
{
    float param = u;
    float param_1 = 124.15000152587890625;
    float param_2 = 125.0500030517578125;
    float param_3 = 131.5;
    float param_4 = 132.100006103515625;
...
float zoomAt(thread const float& u)
{
    ...
    float param_3 = 131.5;
    float param_4 = 132.8000030517578125;
...
{
    float back = 131.5 + (3.0 * fract(h * 7.30000019073486328125));
    ...
    float gone = 124.0 + (0.5 * fract(h * 3.7000000476837158203125));
    float glow = 0.119999997317790985107421875 * fast::max(1.0 - (((u - gone) - 0.0599999986588954925537109375) / 0.699999988079071044921875), 0.0);
    return mix(1.0, fast::max(lit, glow), smoothstep(gone, gone + 0.0599999986588954925537109375, u));
}
...
float tubeLevel(thread const float2& cell, thread const float& t)
{
    ...
    float u = mod(t, 150.0);
    float power = 1.0;
    if ((u > 124.0) && (u < 135.649993896484375))
    {
        float _2065;
        ...
        else
        {
            float param_2 = u;
            float param_3 = h;
            _2065 = tubeMains(param_2, param_3);
        }
        power = _2065;
        if (power == 0.0)
        {
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2390 / 4.0);
...
float figureShows(thread const float& u)
{
    float param = u;
    float param_1 = 124.0;
    float param_2 = 124.40000152587890625;
    float param_3 = 131.5;
    float param_4 = 131.649993896484375;
    return bump(param, param_1, param_2, param_3, param_4);
}
...
Wrong wrongOf(thread const float& lap)
{
    ...
    float a = floor(cheap21(param) * 3.0);
    float2 param_1 = float2(lap, 17.0);
    float b = floor(cheap21(param_1) * 3.0);
    ...
    float param_13 = b;
    float param_14 = two;
    return Wrong{ picked(param_3, param_4, param_5, param_6), picked(param_7, param_8, param_9, param_10), picked(param_11, param_12, param_13, param_14) };
}
...
float figureDist(thread float2& p, thread const Wrong& w)
{
    ...
    d = fast::min(d, segment(param_6, param_7, param_8) - 0.17000000178813934326171875);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3770)
{
    float2 res = float2(_3770.iResolution[0], _3770.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3770.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3770.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_18;
    float3 param_19;
    float _4136 = trace(param_16, param_17, param_18, param_19);
    int id = param_18;
    float3 n = param_19;
    float dist = _4136;
    float3 p = ro + (rd * dist);
    ...
    float3 param_25;
    float3 _4170 = surface(param_20, param_21, param_22, param_23, param_24, param_25);
    ...
    if (shows > 0.0)
    {
        ...
        bool _4286 = (tHit > 0.0) && (tHit < dist);
        bool _4299;
        if (_4286)
        {
            _4299 = abs(local.x) < ((0.4000000059604644775390625 * scale) + (5.0 * px));
        }
        else
        {
            _4299 = _4286;
        ...
        if (_4299)
        {
            _4305 = local.y > (-0.100000001490116119384765625);
        }
        else
        {
            _4305 = _4299;
        ...
        if (_4305)
        {
            _4316 = local.y < (((1.75 * scale) + 0.119999997317790985107421875) + 0.100000001490116119384765625);
        }
        else
        {
            _4316 = _4305;
        ...
        if (_4316)
        {
            float2 param_31 = float2(line, fseed.x);
            float tear = cheap21(param_31);
            local.x += ((5.0 * px) * (tear - 0.5));
            float aa = 4.0 * px;
            float2 param_32 = local;
            Wrong param_33 = wrong;
            float _4347 = figureDist(param_32, param_33);
            float cover = shows * smoothstep(aa, -aa, _4347);
            float2 param_34 = float2(line, fseed.y);
            cover *= (1.0 - (0.60000002384185791015625 * cheap21(param_34)));
            if (cover > 0.0)
            {
                float3 param_35 = hit;
                float3 param_36 = face;
                float param_37 = tNow;
                float3 lit = float3(0.0199999995529651641845703125) * (lighting(param_35, param_36, param_37) + float3(0.01200000010430812835693359375));
    ...
    C += (dyiq.yz * 6.0);
    float2 param_38 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_38) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_39 = chromaAt + fseed;
    float2 blotch = (_noise2(param_39) * 2.0) - float2(1.0);
    ...
    float2 param_40 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_40);
    float snowGate = 0.930000007152557373046875 - (storm * 0.119999997317790985107421875);
    float2 param_41 = (lumaCell + fseed) + float2(3.0);
    float2 param_42 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(snowGate, cheap21(param_41)) - step(snowGate, cheap21(param_42));
    ...
    float2 param_43 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_43);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_44 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_44);
...
fragment main0_out main0(constant Globals& _3770 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3770);
    out._fragColor = param;
    return out;
}
"""#
