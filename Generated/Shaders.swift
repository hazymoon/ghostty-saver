...
// shaders-sha256: fdbd3a3ded369fc86ce0e8a6690dfd5d28e91e364b716ae4c0bde455bdbbdbe2
// prefix-ref: v1.3.1
...
{
    float2 c = mod(cell, float2(8.0)) - float2(5.0, 3.0);
    bool _382 = c.x >= 0.0;
    bool _388;
    if (_382)
    {
...
float3 lighting(thread const float3& p, thread const float3& n, thread const float& t)
{
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            bounced += (((level0 * 0.85000002384185791015625) * window) / (1.0 + (d2 * 0.300000011920928955078125)));
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2282 / 4.0);
                if (wallCovers(param_12, param_13))
                {
                    continue;
                }
            }
            direct += ((((level0 * lambert) * 2.400000095367431640625) * window) / (1.0 + (d2 * 0.20000000298023223876953125)));
        }
    }
...
static inline __attribute__((always_inline))
float ceilingFill(thread const float3& p, thread const float3& n)
{
    float tile = 32.0;
    float2 centre = float2(26.0, 18.0);
    float2 half_ = float2(6.0);
    float2 d = mod((p.xz - centre) + float2(tile * 0.5), float2(tile)) - float2(tile * 0.5);
    float outside = length(fast::max(abs(d) - half_, float2(0.0)));
    float hemisphere = 0.5 + (0.5 * n.y);
    return (0.5 * hemisphere) * smoothstep(0.0, 4.0, outside);
}
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3437)
{
    float2 res = float2(_3437.iResolution[0], _3437.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3437.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3437.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_14;
    float3 param_15;
    float _3751 = trace(param_12, param_13, param_14, param_15);
    int id = param_14;
    float3 n = param_15;
    float dist = _3751;
    float3 p = ro + (rd * dist);
    ...
    float3 param_18 = n;
    float param_19 = tNow;
    float param_20 = footprint;
    float3 param_21;
    float3 _3785 = surface(param_16, param_17, param_18, param_19, param_20, param_21);
    float3 emission = param_21;
    float3 albedo = _3785;
    float3 param_22 = p;
    float3 param_23 = n;
    float param_24 = tNow;
    float3 param_25 = p;
    float3 param_26 = n;
    float3 col = (albedo * ((lighting(param_22, param_23, param_24) + (float3(1.0, 0.980000019073486328125, 0.819999992847442626953125) * ceilingFill(param_25, param_26))) + float3(0.02500000037252902984619140625))) + emission;
    ...
    C += (dyiq.yz * 6.0);
    float2 param_27 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_27) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_28 = chromaAt + fseed;
    float2 blotch = (_noise2(param_28) * 2.0) - float2(1.0);
    ...
    float2 param_29 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_29);
    float snowGate = 0.930000007152557373046875 - (storm * 0.119999997317790985107421875);
    float2 param_30 = (lumaCell + fseed) + float2(3.0);
    float2 param_31 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(snowGate, cheap21(param_30)) - step(snowGate, cheap21(param_31));
    ...
    float2 param_32 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_32);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_33 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_33);
...
fragment main0_out main0(constant Globals& _3437 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3437);
    out._fragColor = param;
    return out;
}
"""#
