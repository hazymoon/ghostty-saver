...
// shaders-sha256: 20fbf4229755b1b021e68ee3de35abf625a00dca374df245464ee4bda5f9b683
// prefix-ref: v1.3.1
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _678 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _678 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _688 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _688 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_678, _688);
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _733 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _733;
        ...
        if (axisX)
        {
            ...
            {
                _785 = cell - float2(1.0, 0.0);
            }
            float2 wallCell = _785;
        ...
        else
        {
            ...
            {
                _808 = cell - float2(0.0, 1.0);
            }
            float2 wallCell_1 = _808;
        ...
        if (wallCovers(param_8, param_9))
        {
            ...
            if (axisX)
            {
                _833 = float3(-_step.x, 0.0, 0.0);
            }
            else
            {
                _833 = float3(0.0, 0.0, -_step.y);
            }
            normal = _833;
            return exit;
        }
...
float3 surface(thread const float3& p, thread const int& id, thread const float3& n, thread const float& t, thread float3& emission)
{
    ...
    float top = 1.0 - (0.1500000059604644775390625 * smoothstep(2.2000000476837158203125, 2.5, p.y));
...
float3 lighting(thread const float3& p, thread const float3& n, thread const float& t)
{
    ...
    for (int dz = -2; dz <= 2; dz++)
    {
        for (int dx = -2; dx <= 2; dx++)
        {
            ...
            float2 param_2 = c;
            float2 param_3 = c;
            float3 lp = float3(tubeCentre(param_2).x, 2.4500000476837158203125, tubeCentre(param_3).y);
            float3 l = lp - p;
            float d2 = dot(l, l);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _1401)
{
    float param = _1401.iTime;
    float t = droppedFrames(param);
    float2 res = float2(_1401.iResolution[0], _1401.iResolution[1]);
    float mask = 1.0;
    ...
    float param_2 = _1401.iTime;
    float band = trackingBand(param_1, param_2);
    float param_3 = floor(screenY * 90.0) + floor(_1401.iTime * 30.0);
    ...
    float param_4 = floor(_1401.iTime * 60.0);
    float headShift = (headSwitch * 0.02999999932944774627685546875) * (hash11(param_4) - 0.5);
    float wobble = (0.00120000005699694156646728515625 * sin((screenY * 37.0) + (_1401.iTime * 21.0))) * (0.5 + (0.5 * sin(_1401.iTime * 0.699999988079071044921875)));
    ...
    float3 ro = float3(((pos2.x * 4.0) + 2.0) + sway.x, 1.5499999523162841796875 + bob, ((pos2.y * 4.0) + 2.0) + sway.y);
    ...
    int param_18;
    float3 param_19;
    float _1789 = trace(param_16, param_17, param_18, param_19);
    int id = param_18;
    float3 n = param_19;
    float dist = _1789;
    float3 p = ro + (rd * dist);
    ...
    float param_23 = t;
    float3 param_24;
    float3 _1809 = surface(param_20, param_21, param_22, param_23, param_24);
    ...
    float2 param_28 = floor(fragCoord / float2(1.5)) + float2(fract(_1401.iTime * 7.0) * 100.0);
    float grain = hash21(param_28);
    col += float3((grain - 0.5) * 0.0900000035762786865234375);
    float2 param_29 = float2(floor(screenY * 240.0), floor(_1401.iTime * 30.0));
    ...
    float2 param_30 = fragCoord + float2(fract(_1401.iTime * 13.0) * 50.0);
    ...
    float param_31 = floor(_1401.iTime * 24.0);
...
fragment main0_out main0(constant Globals& _1401 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _1401);
    out._fragColor = param;
    return out;
}
"""#
