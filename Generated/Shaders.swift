...
// shaders-sha256: 935b17b9b8774b857dbb05cab11d8b99065061030c64771953950577829dd01e
// prefix-ref: v1.3.1
...
constant spvUnsafeArray<float4, 7> _588 = spvUnsafeArray<float4, 7>({ float4(30.0, 10.0, 2.0, 1.0), float4(106.0, 10.0, 2.0, 1.0), float4(23.229999542236328125, 8.770000457763671875, 2.0, 0.449999988079071044921875), float4(38.0, 20.8899993896484375, 2.0, 0.449999988079071044921875), float4(126.0, 9.0, 0.60000002384185791015625, 1.0), float4(56.8899993896484375, 22.0, 2.5, -0.300000011920928955078125), float4(135.0, 12.0, 1.5, -0.300000011920928955078125) });
constant spvUnsafeArray<float4, 2> _795 = spvUnsafeArray<float4, 2>({ float4(6.0, 5.0, 40.0, 6.5), float4(3.0, 6.0, 124.8000030517578125, 8.0) });
constant spvUnsafeArray<float2, 31> _963 = spvUnsafeArray<float2, 31>({ float2(1.0, 0.0), float2(1.0), float2(1.0, 2.0), float2(2.0), float2(3.0, 2.0), float2(4.0, 2.0), float2(4.0, 3.0), float2(4.0), float2(4.0, 5.0), float2(4.0, 6.0), float2(4.0, 7.0), float2(4.0, 8.0), float2(4.0, 9.0), float2(4.0, 10.0), float2(3.0, 10.0), float2(2.0, 10.0), float2(1.0, 10.0), float2(0.0, 10.0), float2(-1.0, 10.0), float2(-2.0, 10.0), float2(-3.0, 10.0), float2(-3.0, 9.0), float2(-3.0, 8.0), float2(-3.0, 7.0), float2(-3.0, 6.0), float2(-2.0, 6.0), float2(-1.0, 6.0), float2(0.0, 6.0), float2(1.0, 6.0), float2(1.0, 7.0), float2(1.0, 8.0) });
constant spvUnsafeArray<int, 64> _1271 = spvUnsafeArray<int, 64>({ 0, 10, 0, 0, 10, 10, 0, 0, 0, 10, 0, 0, 10, 10, 0, 0, 5, 13, 5, 5, 14, 9, 5, 5, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 0, 0, 0, 0, 11, 4, 0, 0, 5, 6, 0, 0, 10, 3, 5, 5, 0, 10, 0, 0, 10, 10, 0, 0 });
...
float2 waypoint(thread const float& k)
{
    ...
    return _963[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
void smoothPath(thread const float& s, thread float2& pos, thread float2& dir, thread float2& turn)
{
    ...
    for (float k = k0; k <= k1; k += 1.0)
    {
        ...
        for (int part = 0; part < 2; part++)
        {
            ...
            {
                _1042 = fast::max(a, 0.0);
            }
            float ta = _1042;
            if (part == 0)
            {
                _1053 = fast::min(b, 0.0);
            }
            else
            {
                _1053 = b;
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1368 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1368 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1379 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1379 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1368, _1379);
    float3 _1465;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1465 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1465 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1465;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1271[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1512 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1512 = float2(0.0);
        }
        float2 owner = cell - _1512;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1683 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1683 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1683;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1707 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1707;
    ...
    if (rd.x == 0.0)
    {
        _1751 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1751 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1761 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1761 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1751, _1761);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1815 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1815;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1830 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1830;
...
{
    float2 param = cell;
    bool _1933 = !inBlackout(param);
    bool _1941;
    if (_1933)
    {
        float2 param_1 = cell;
        _1941 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1941 = _1933;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2365 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _3814)
{
    float2 res = float2(_3814.iResolution[0], _3814.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_3814.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _3814.iTime - ((fields - fieldNo) / 59.939998626708984375);
    ...
    int param_15;
    float3 param_16;
    float _4142 = trace(param_13, param_14, param_15, param_16);
    int id = param_15;
    float3 n = param_16;
    float dist = _4142;
    float3 p = ro + (rd * dist);
    ...
    float3 param_19 = n;
    float param_20 = tNow;
    float param_21 = footprint;
    float3 param_22;
    float3 _4176 = surface(param_17, param_18, param_19, param_20, param_21, param_22);
    float3 emission = param_22;
    float3 albedo = _4176;
    float3 param_23 = p;
    float3 param_24 = n;
    float param_25 = tNow;
    float3 col = (albedo * (lighting(param_23, param_24, param_25) + float3(0.02500000037252902984619140625))) + emission;
    ...
    float param_26 = lapNow;
    float shows = figureShows(param_26);
    if (shows > 0.0)
    {
        float param_27 = floor(tNow / 150.0);
        Wrong wrong = wrongOf(param_27);
        ...
        bool _4284 = (tHit > 0.0) && (tHit < dist);
        bool _4294;
        if (_4284)
        {
            _4294 = abs(local.x) < (0.4000000059604644775390625 * scale);
        }
        else
        {
            _4294 = _4284;
        ...
        if (_4294)
        {
            _4300 = local.y > (-0.100000001490116119384765625);
        }
        else
        {
            _4300 = _4294;
        ...
        if (_4300)
        {
            _4311 = local.y < (((1.75 * scale) + 0.119999997317790985107421875) + 0.100000001490116119384765625);
        }
        else
        {
            _4311 = _4300;
        ...
        if (_4311)
        {
            float aa = tHit / (1.25 * frame.y);
            float2 param_28 = local;
            Wrong param_29 = wrong;
            float _4329 = figureDist(param_28, param_29);
            float cover = shows * smoothstep(aa, -aa, _4329);
            if (cover > 0.0)
            {
                float3 param_30 = hit;
                float3 param_31 = face;
                float param_32 = tNow;
                float3 lit = float3(0.0199999995529651641845703125) * (lighting(param_30, param_31, param_32) + float3(0.01200000010430812835693359375));
    ...
    C += (dyiq.yz * 6.0);
    float2 param_33 = float2(line, fseed.x);
    float phase = ((0.14000000059604644775390625 * (cheap21(param_33) - 0.5)) * 2.0) + (bandEdge * 1.2000000476837158203125);
    C = float2x2(float2(cos(phase), sin(phase)), float2(-sin(phase), cos(phase))) * C;
    float2 param_34 = chromaAt + fseed;
    float2 blotch = (_noise2(param_34) * 2.0) - float2(1.0);
    ...
    float2 param_35 = floor(fragCoord / float2(2.5)) + fseed;
    float grain = cheap21(param_35);
    float snowGate = 0.930000007152557373046875 - (storm * 0.119999997317790985107421875);
    float2 param_36 = (lumaCell + fseed) + float2(3.0);
    float2 param_37 = (lumaCell + fseed.yx) + float2(5.0);
    float snow = step(snowGate, cheap21(param_36)) - step(snowGate, cheap21(param_37));
    ...
    float2 param_38 = float2(floor(screenY * 240.0), fseed.y);
    float bandNoise = cheap21(param_38);
    Y = mix(Y, 0.699999988079071044921875 + (0.300000011920928955078125 * bandNoise), band * 0.800000011920928955078125);
    float2 param_39 = floor(fragCoord) + fseed.yx;
    float headSnow = cheap21(param_39);
...
fragment main0_out main0(constant Globals& _3814 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _3814);
    out._fragColor = param;
    return out;
}
"""#
