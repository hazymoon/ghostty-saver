...
// shaders-sha256: 054823225355100b44384a30aacf6f6b1ef9385f00c5f49233c17702594cda56
// prefix-ref: v1.3.1
...
float walkSpeed()
{
    return 0.22388060390949249267578125;
}
...
{
    float param = start;
    float param_1 = start + 2.0;
    float param_2 = u;
    float param_3 = (start + 10.0) - 2.0;
    float param_4 = start + 10.0;
    float param_5 = u;
...
float walked(thread const float& u)
{
    ...
    float param_2 = u;
    float param_3 = 106.0;
    return walkSpeed() * ((u - stoodStill(param, param_1)) - stoodStill(param_2, param_3));
}

static inline __attribute__((always_inline))
float pace(thread const float& u)
{
    float p1 = smoothstep(30.0, 32.0, u) - smoothstep(38.0, 40.0, u);
    float p2 = smoothstep(106.0, 108.0, u) - smoothstep(114.0, 116.0, u);
    return (1.0 - p1) - p2;
}
...
float2 waypoint(thread const float& k)
{
    ...
    return _710[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
Pose cameraPose(thread const float& t)
{
    ...
    float param_27 = stepNo;
    float param_28 = stepNo + 1.0;
    float amp = (mix(stepSize(param_27), stepSize(param_28), su) * gait) * braced;
    float arch = 0.800000011920928955078125;
    float strike = (0.0040000001899898052215576171875 * gait) * braced;
    float bobLow = (((-0.02500000037252902984619140625) * amp) * arch) * cos(6.283185482025146484375 * su);
    float param_29 = sinceStrike;
    float param_30 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + (strike * ring(param_29, param_30));
    float param_31 = sinceStrike;
    float param_32 = 0.070000000298023223876953125;
    float bobRate = ((((0.02500000037252902984619140625 * amp) * arch) * stepRate) * (((6.283185482025146484375 * sin(6.283185482025146484375 * su)) + (2.51327419281005859375 * sin(12.56637096405029296875 * su))) + (0.94247782230377197265625 * sin(18.84955596923828125 * su)))) + ((strike * ringRate(param_31, param_32)) * moving);
    ...
    float param_33 = sinceStrike;
    float param_34 = 0.07999999821186065673828125;
    float kick = ((0.001700000022538006305694580078125 * gait) * braced) * ring(param_33, param_34);
    float param_35 = sinceStrike;
    float param_36 = 0.07999999821186065673828125;
    float kickRate = (((0.001700000022538006305694580078125 * gait) * braced) * ringRate(param_35, param_36)) * moving;
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1117 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1117 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1128 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1128 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1117, _1128);
    float3 _1214;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1214 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1214 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1214;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1020[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1261 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1261 = float2(0.0);
        }
        float2 owner = cell - _1261;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1432 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1432 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1432;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1456 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1456;
    ...
    if (rd.x == 0.0)
    {
        _1500 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1500 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1510 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1510 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1500, _1510);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1564 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1564;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1579 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1579;
...
{
    float2 param = cell;
    bool _1681 = !inBlackout(param);
    bool _1689;
    if (_1681)
    {
        float2 param_1 = cell;
        _1689 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1689 = _1681;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_2008 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2995)
{
    float2 res = float2(_2995.iResolution[0], _2995.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_2995.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _2995.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    ...
    int param_10;
    float3 param_11;
    float _3299 = trace(param_8, param_9, param_10, param_11);
    int id = param_10;
    float3 n = param_11;
    float dist = _3299;
    float3 p = ro + (rd * dist);
    ...
    float param_15 = tNow;
    float3 param_16;
    float3 _3319 = surface(param_12, param_13, param_14, param_15, param_16);
...
fragment main0_out main0(constant Globals& _2995 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _2995);
    out._fragColor = param;
    return out;
}
"""#
