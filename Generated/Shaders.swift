...
// shaders-sha256: d434c835f88d48572f052e0042d8221982793ebd53f7472191a43b3eacad0865
// prefix-ref: v1.3.1
...
float2 waypoint(thread const float& k)
{
    ...
    return _694[int(k - (lap * 30.0))] + float2(0.0, lap * 8.0);
}
...
Pose cameraPose(thread const float& t)
{
    ...
    float param_26 = stepNo;
    float param_27 = stepNo + 1.0;
    float amp = mix(stepSize(param_26), stepSize(param_27), su) * gait;
    ...
    float param_28 = sinceStrike;
    float param_29 = 0.070000000298023223876953125;
    float bobHigh = ((((-0.02500000037252902984619140625) * amp) * arch) * ((0.20000000298023223876953125 * cos(12.56637096405029296875 * su)) + (0.0500000007450580596923828125 * cos(18.84955596923828125 * su)))) + ((0.0040000001899898052215576171875 * gait) * ring(param_28, param_29));
    float param_30 = sinceStrike;
    float param_31 = 0.070000000298023223876953125;
    float bobRate = ((((0.02500000037252902984619140625 * amp) * arch) * stepRate) * (((6.283185482025146484375 * sin(6.283185482025146484375 * su)) + (2.51327419281005859375 * sin(12.56637096405029296875 * su))) + (0.94247782230377197265625 * sin(18.84955596923828125 * su)))) + (((0.0040000001899898052215576171875 * gait) * ringRate(param_30, param_31)) * moving);
    ...
    float param_33 = 0.07999999821186065673828125;
    float kick = (0.001700000022538006305694580078125 * gait) * ring(param_32, param_33);
    float param_34 = sinceStrike;
    float param_35 = 0.07999999821186065673828125;
    float kickRate = ((0.001700000022538006305694580078125 * gait) * ringRate(param_34, param_35)) * moving;
    ...
    float3 known = (head + shift) + float3(0.0, (0.75 * bobLow) + (0.5 * bobHigh), 0.0);
...
float hitPillars(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread float3& normal)
{
    ...
    if (rd.x == 0.0)
    {
        _1101 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1101 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1112 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1112 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1101, _1112);
    float3 _1198;
    for (int i = 0; i < 4; i++)
    {
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (tmin.x > tmin.y)
            {
                _1198 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1198 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1198;
        }
    }
...
bool sideOpen(thread const float2& cell, thread const int& bit)
{
    ...
    int mask = _1004[(int(c.y) * 8) + int(c.x)];
    return (mask & bit) != 0;
}
...
float hitWalls(thread const float3& ro, thread const float3& rd, thread const float2& cell, thread const float2& inv, thread const float2& _step, thread float3& normal)
{
    ...
    for (int e = 0; e < 2; e++)
    {
        ...
        if (back < 0.0)
        {
            _1245 = select(float2(0.0, 1.0), float2(1.0, 0.0), bool2(axisX));
        }
        else
        {
            _1245 = float2(0.0);
        }
        float2 owner = cell - _1245;
        float2 param = owner;
        bool param_1 = axisX;
        ...
        if (((enter < exit) && (enter > 0.0)) && (enter < best))
        {
            ...
            if (onFace == axisX)
            {
                _1416 = float3(-sign(rd.x), 0.0, 0.0);
            }
            else
            {
                _1416 = float3(0.0, 0.0, -sign(rd.z));
            }
            normal = _1416;
        }
    }
...
float trace(thread const float3& ro, thread const float3& rd, thread int& id, thread float3& normal)
{
    ...
    {
        _1440 = (2.7000000476837158203125 - ro.y) / rd.y;
    }
    float planeT = _1440;
    ...
    if (rd.x == 0.0)
    {
        _1484 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1484 = rd.x;
    ...
    if (rd.z == 0.0)
    {
        _1494 = 9.9999999747524270787835121154785e-07;
    }
    else
    {
        _1494 = rd.z;
    }
    float2 inv = float2(1.0) / float2(_1484, _1494);
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        float3 param_1 = rd;
        float2 param_2 = cell;
        float _1548 = hitPillars(param, param_1, param_2, param_3);
        float3 pn = param_3;
        float pt = _1548;
        float3 param_4 = ro;
        float3 param_5 = rd;
        ...
        float _1563 = hitWalls(param_4, param_5, param_6, param_7, param_8, param_9);
        float3 wn = param_9;
        float wt = _1563;
...
{
    float2 param = cell;
    bool _1665 = !inBlackout(param);
    bool _1673;
    if (_1665)
    {
        float2 param_1 = cell;
        _1673 = tubeHash(param_1) <= 0.7200000286102294921875;
    }
    else
    {
        _1673 = _1665;
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            if ((dx != 0) || (dz != 0))
            {
                ...
                int param_12 = kind;
                float param_13 = fract(_1992 / 4.0);
...
static inline __attribute__((always_inline))
void mainImage(thread float4& fragColor, thread const float2& fragCoord, constant Globals& _2957)
{
    float2 res = float2(_2957.iResolution[0], _2957.iResolution[1]);
    float mask = 1.0;
    ...
    uv.y = -uv.y;
    float fields = mod(_2957.iTime, 3600.0) * 59.939998626708984375;
    ...
    float now = _2957.iTime - ((fields - fieldNo) / 59.939998626708984375);
    float param = now;
    ...
    int param_10;
    float3 param_11;
    float _3261 = trace(param_8, param_9, param_10, param_11);
    int id = param_10;
    float3 n = param_11;
    float dist = _3261;
    float3 p = ro + (rd * dist);
    ...
    float param_15 = tNow;
    float3 param_16;
    float3 _3281 = surface(param_12, param_13, param_14, param_15, param_16);
...
fragment main0_out main0(constant Globals& _2957 [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    float2 param_1 = gl_FragCoord.xy;
    float4 param;
    mainImage(param, param_1, _2957);
    out._fragColor = param;
    return out;
}
"""#
